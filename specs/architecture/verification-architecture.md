...
| VP-124 | Backfill-Split Is Atomic Under Interruption, Idempotent Against an Already-Sharded Artifact, Recovery-Confirmation Is Exact-Manifest-Based Never a Byte-Prefix Heuristic (EXTENDED v1.6: SAFE/DANGEROUS/AMBIGUOUS disposition determined SOLELY by exact whole-file (length, SHA-256) comparison against the Backfill Recovery Manifest — a repeated-prefix content case, EC-009, is correctly classified SAFE, and a match against neither manifest pair fails loud as E-SHD-011, EC-010), and the DANGEROUS-Window Heal's Slice Is Manifest-Derived and Manifest-Verified Before Every Write (EXTENDED v1.7: offset = original_bytes - final_bytes derived from the Manifest alone, never a summed shard-index bytes_at_seal; the slice is NEVER written unless sliced.len()==final_bytes AND sha256(sliced)==final_sha256; a mismatch, EC-011, fails loud as E-SHD-012 with no write; the heal's own write receives the same post-hoc disk read-back PC6(c)/INV4 already require) | integration | BC-1.18.008 PC5(Recovery-Confirmation Rule, NEW v1.6; Manifest-Authoritative Slice-and-Verify Rule, NEW v1.7)/PC6(c)(disk-read-back scope extended v1.7)/INV3(tightened v1.6; extended v1.7)/INV4(disk-read-back scope extended v1.7)/EC-003/EC-004/EC-009(NEW v1.6)/EC-010(NEW v1.6)/EC-011(NEW v1.7) | draft |
