...
6. **MUST cover the SS-05/SS-06 second-level sub-split within the SAME one-time migration
   ...
   sub-sharding as part of this SAME one-time B2 migration operation (independently of mechanism A's activation schedule — per BC-1.18.011 Precondition 4 and ADR-052 §Decision 1, the two migrations activate independently and may run in any order).
...
**v1.12 (cluster-4 hardening — Obs-A E-SHD-014 counter-divergence guard; Obs-B withdrawn in v1.13):** §Decision 7 B1 subsection amended with the Obs-A counter-divergence guard bullet per `.factory/cycles/v1.0-brownfield-backfill/s2502-cluster4-hardening-design.md`. B1 handler MUST check `RotationReport.mutated` after every `Ok(report)` return from `rotate_changelog_at`; `mutated==false` returns `HookResult::Error(E-SHD-014)` instead of `Block`. Erratum in same burst: original draft cited `E-SHD-008`, already allocated to BackstopProbeFailed (BC-1.18.006 EC-019); corrected to `E-SHD-014`, error-taxonomy.md v1.15 authoritative. BC-side anchors: BC-1.18.009 EC-008, Inv-5 (v1.6). An Obs-B crash-recovery self-heal bullet was also added in this burst but withdrawn immediately in v1.13 (see below).

**v1.13 (Obs-B withdrawal — adv re-cascade pass-1 F-C4H-P1-002):** Withdrew the §Decision 7 Obs-B "B1 crash-recovery partial failure — transparent self-heal, no new error code" bullet added in v1.12. The pure byte-level tail-match mechanism that bullet described is unimplementable: the guard cannot distinguish a crash-retry (archive tail matches current `move_items` by definition) from a legitimate later rotation whose `move_items` coincide with the archived tail (adv re-cascade pass-1 F-C4H-P1-002). The implementer's sentinel-file workaround was independently broken (F-C4H-P1-001 CRITICAL, sentinel never reset) and architect-rejected. B1 archive crash-atomicity is deferred to follow-up story S-25.05. **The Obs-A E-SHD-014 counter-divergence guard bullet is RETAINED unchanged.** Withdrawal-class; no decision reversed for Obs A. Refs: adv re-cascade pass-1 F-C4H-P1-001, F-C4H-P1-002, S-25.05.
