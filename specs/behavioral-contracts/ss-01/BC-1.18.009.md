...
| EC-009 (NEW — cluster-4 hardening, SEC-002 / E-SHD-015 archive-path guard; D-1212-DRIFT-001 spec catch-up) | Item-count trigger fires (`current_item_count + 1 > N`) but the B1 handler cannot derive the evergreen archive sibling path because `target_path.parent()` returns `None` — the `target_path` has no directory component (e.g. a bare filename such as `"BC-INDEX.md"` with no leading directory, rather than an absolute path). Without the SEC-002 guard, the prior `target_path.parent().unwrap_or_else(\|\| Path::new("."))` fallback would have silently resolved the archive destination into the process's CWD (SEC-002, CWE-252 silent-CWD-fallback). In practice fires only in misconfigured or test-harness environments: a correctly-configured dispatcher always supplies an absolute `target_path` derived from the shard config entry (itself validated by `validate_entry`'s SEC-002/CWE-22 `artifact_stem` check), so `parent()` always returns `Some`. Structurally unreachable on POSIX via the integration test path (any path carrying a file stem necessary to match a `[[shard]]` entry has `Some(parent)` on POSIX; only the bare root `/` has `parent() == None`, and `/` has no file stem). | `HookResult::Error` with message `E-SHD-015: cannot resolve archive path — target_path '<path>' has no parent directory component` — byte-for-byte matching the shipped `Display` and `error-taxonomy.md` v1.17 `E-SHD-015` row; the triggering write is NOT applied; the frontmatter `changelog:` sequence is left byte-identical to its pre-attempt state (fail-loud, no partial state authoritative); this is a BLOCKING fail-loud Error (`block_intent=true`, `exit_code=2`, `on_error=Block`), consistent with EC-003 / EC-008's blocking treatment; fires BEFORE `rotate_changelog_at` is invoked, during archive path construction — distinct from EC-003 (rotate invocation failure, `E-SHD-004`, fires AFTER the rotate call is attempted) and EC-008 (counter-divergence guard, `E-SHD-014`, fires AFTER rotate returns `Ok(mutated=false)`) |
...
| **NEW (cluster-4 hardening, EC-009, SEC-002 / E-SHD-015 guard; D-1212-DRIFT-001 spec catch-up).** Fixture: item-count trigger fires; `target_path` is a bare filename with no directory component (`Path::new("BC-INDEX.md")`), so `target_path.parent()` returns `None`; `rotate_changelog_at` is NOT invoked. Source-scan coverage (structurally unreachable via integration on POSIX: any path that reaches the `FrontmatterChangelogArray` arm carries a file stem necessary to match a `[[shard]]` entry, and on POSIX any such path has `Some(parent)`). Discriminating test: `test_BC_1_18_009_SEC002_shard_manager_contains_e_shd_015_none_parent_guard` (`crates/factory-dispatcher/tests/bc_1_18_009_b1_rotate_test.rs`) — two checks: (1) `E-SHD-015` is present in `shard_manager.rs` source; (2) pre-fix `target_path.parent().unwrap_or_else` is absent from `shard_manager.rs` source. | `HookResult::Error` with message `E-SHD-015: cannot resolve archive path — target_path '<path>' has no parent directory component` (byte-for-byte matching shipped `Display` and `error-taxonomy.md` v1.17); frontmatter `changelog:` byte-identical to pre-attempt state; BLOCKING Error (`block_intent=true`, `exit_code=2`, `on_error=Block`) — the write is BLOCKED, not advisory; `rotate_changelog_at` is NOT called (guard fires before the rotate call, during archive path construction) | error (SEC-002 / CWE-252 guard) |
...
|---------|------|--------|--------|
| 1.8 | 2026-09-12 | product-owner | Drift Item D-1212-DRIFT-001 post-merge spec catch-up (no code change — documents already-merged behavior from S-25.02 cluster-4, develop @ ebd16f79). **Added EC-009** (SEC-002 / E-SHD-015 archive-path guard): item-count trigger fires but `target_path.parent()` returns `None` — a bare filename with no directory component — which without the guard would silently resolve the archive destination to the process's CWD (SEC-002, CWE-252 silent-CWD-fallback); the gate returns `HookResult::Error(E-SHD-015)` fail-loud BEFORE invoking `rotate_changelog_at`, leaving the frontmatter `changelog:` byte-identical to its pre-attempt state; structurally unreachable on POSIX in practice (any path that reaches the `FrontmatterChangelogArray` arm carries a file stem, therefore has `Some(parent)` on POSIX); discriminating test: `test_BC_1_18_009_SEC002_shard_manager_contains_e_shd_015_none_parent_guard` (source-scan, `crates/factory-dispatcher/tests/bc_1_18_009_b1_rotate_test.rs`). Added matching Canonical Test Vector for EC-009. No postcondition, invariant, VP, or contract-behavior change — documentary spec↔code alignment for already-merged, tested, PR-reviewed behavior. |
| 1.7 | 2026-09-11 | product-owner | Withdrew Obs-B crash-recovery clauses — unsound design per adv re-cascade pass-1 F-C4H-P1-001 (CRITICAL — sentinel never reset, duplicates on rotation #2+) and F-C4H-P1-002 (pure byte-level tail-match design is internally unimplementable — cannot distinguish crash-retry from a legitimate later rotation with coincident items). Proper cross-file crash-atomicity deferred to S-25.05 (backlog, E-25). **Removed from live contract:** Invariant Inv-6 (crash-idempotent archive boundary), PC5 Obs-B no-duplicate addendum ("The archive NEVER contains duplicate items across rotation cycles; a partial rotation cycle (archive written, source-write failed) is detected and resolved idempotently on the next invocation (see Invariant Inv-6)"), Obs-B crash-recovery Canonical Test Vector, VP-112 row (which discharged Inv-6 — VP-112 no longer discharges anything in this BC). **Retained intact:** EC-008 (counter-divergence guard), Invariant Inv-5 (`Ok(report)` with `!report.mutated` after fired trigger → `Error(E-SHD-014)`, never `Block`), PC6 Obs-A addendum (E-SHD-014 blocking guard), EC-008 CTV, VP-126/VP-131 Obs-A facet references. Obs-A E-SHD-014 guard fully intact. |
