...
modified: [v1.0-brownfield-backfill]
deprecated: null
deprecated_by: null
replacement: null
retired: null
...
3. **The full shard index is published for the complete pre-existing history in the same
   ...
   **Schema-location correction (this amendment, F-C3-P9-004, BLOCKING): the Manifest is an
   ADDITIVE FIELD of the shared `ShardIndex` struct itself — `backfill_manifest:
   Option<BackfillManifest>`, `#[serde(default, skip_serializing_if = "Option::is_none")]` — never
   a side-channel `[backfill_manifest]` TOML table appended outside that struct by a separate,
   independently-maintained read/write path.** v1.6 (F-C3-P6-001) originally specified the Manifest
   as a hand-appended table read and written by dedicated helper functions
   (`read_backfill_manifest`/`write_shard_index_for_backfill`) entirely outside the `ShardIndex`
   struct that every OTHER `ShardIndex`-touching function in this module (BC-1.18.006's ongoing
   per-write roll, both self-heal reconciliation paths) loads, mutates, and re-serializes as a
   whole. That choice was justified at the time as avoiding a breaking struct-literal change at
   pre-existing test call sites (a REQUIRED new field would force every existing `ShardIndex {
   ... }` literal to be updated) — but this reasoning overlooked that an OPTIONAL,
   `#[serde(default)]` field imposes no such requirement, exactly the SAME additive pattern already
   established for `sealed_retroactively`/`oversized_record`/`is_preamble_shard`/`records` on
   `ShardIndexEntry`, and for `retention_count` on `ShardIndex` ITSELF (BC-1.18.007 Postcondition
   1) — a direct precedent for a sibling BC extending this exact shared struct with a new optional
   field with zero special-case plumbing at unrelated call sites. The side-channel design's real
   consequence: every function that loads a `ShardIndex`, mutates it, and writes it back —
   `publish_shard_index_update` (BC-1.18.006's ongoing roll, called on every future over-cap write
   against any artifact, including the four mandatory backfill artifacts once their canonical file
   is a fresh post-backfill "current" file), `self_heal_resume_from_truncate`, and
   `self_heal_reconcile_missing_index_entries` — re-serializes ONLY the `ShardIndex` struct's own
   fields, silently dropping the side-channel `[backfill_manifest]` table on the very first such
   write after a backfill, since that table was never part of the struct those functions actually
   hold in memory. This is a genuine, LIVE data-loss/liveness defect (fresh-eyes PR review on PR
   #831, BLOCKING): once dropped, Invariant 3's SAFE/DANGEROUS/AMBIGUOUS recovery-confirmation
   determination has no Manifest to compare against, and (pre-amendment) permanently fails loud
   with `E-SHD-011` for an artifact that was, in fact, correctly and completely migrated.
   Promoting the Manifest to an ordinary `ShardIndex` field eliminates this failure class
   structurally: every one of the three write sites above already loads, mutates, and
   re-serializes the SAME struct value, so the Manifest field now round-trips through all of them
   automatically, as an ordinary consequence of Rust struct-field serialization — no special-case
   preservation code is required, or permitted, at any of those sites. `BackfillManifestWrapper` and
   `BackfillManifestDocument` (the side-channel serialization/deserialization helpers) and
   `read_backfill_manifest`'s standalone re-parse are RETIRED by this amendment; `load_shard_index`
   alone is now sufficient to read the Manifest, since it is a field of what that function already
   deserializes.

   **Composability with a pre-existing shard set from an ordinary BC-1.18.006 roll (this amendment,
   F-C3-P9-004, BLOCKING).** The schema-location correction above closes the failure mode where a
   backfill-then-roll ordering destroys an already-published Manifest. A SECOND, independent
   failure mode exists for the roll-before-backfill ordering — the expected ordering in practice,
   since the four artifacts this BC exists to shrink are precisely the ones most likely to trip
   BC-1.18.005's cap gate and receive an ordinary roll BEFORE the F4 one-time backfill migration
   ever executes for them: an ordinary roll (`publish_shard_index_update`) synthesizes a FRESH
   `ShardIndex` for an artifact with no pre-existing index (its own `unwrap_or_else` branch), seals
   whatever content was canonical at that moment, and publishes an index with a non-empty
   `[[shard]]` set and (correctly, since no backfill has run yet) `backfill_manifest: None`. When
   the mandatory backfill-split later runs against that SAME artifact, it MUST NOT mistake this
   manifest-less, roll-only index for either "already migrated" (Invariant 3's idempotency
   short-circuit; corrected below to key on Manifest presence, not bare index existence) or for a
   corrupted/ambiguous state — it is neither; the mandatory one-time migration for this artifact
   has simply never run yet, and the resulting shard-index shape (real, sealed rolled shards, no
   Manifest) is a distinct THIRD case Invariant 3's original three-way idempotency disposition list
   never addressed at all. The backfill-split proceeds as a genuine first-ever run, with two
   adjustments to Postcondition 2's packing procedure and this Postcondition's Manifest population:

   1. **`original_bytes`/`original_sha256` are always computed from the canonical file's CURRENT
      on-disk content at the moment `run_mechanism_a_backfill_split` is invoked — never from a
      reconstructed notion of "the full historical monolithic file" the artifact once held.** This
      was already implicitly true whenever no prior roll had happened (the canonical file's current
      content and the full historical monolithic content coincide in that case); this amendment
      states it as the general rule that holds correctly under EITHER ordering. Content a prior
      ordinary roll already sealed off is durably preserved in ITS OWN pre-existing `[[shard]]`
      entries, entirely outside this Manifest's scope — the Manifest only ever needs to account for
      what the canonical file currently holds at the moment this invocation begins, exactly the
      same guarantee BC-1.18.006's own roll mechanism already provides for its own sealed content.
   2. **The backfill-split's own newly-sealed shard entries are APPENDED to the pre-existing
      `[[shard]]` array, with `seq` numbering continuing from `existing_max_seq + 1`** — never
      renumbering, duplicating, or otherwise disturbing the pre-existing rolled entries, which
      remain exactly as BC-1.18.006's roll mechanism published them (append-only, POLICY-1
      consistent).
...
6. **Content-preservation verification is mandatory before the original monolithic file's role is
   ...
   **Extension to the happy-path canonical-truncate write (this amendment, F-C3-P8-002): the same
   disk-read-back discipline applies to Postcondition 5 step (ii)'s ORDINARY, first-time/uninterrupted
   completion — not only to the DANGEROUS-window heal write the F-C3-P7-001 extension above governs.**
   Postcondition 5 step (ii) — `write_atomic`-ing the canonical file to hold only the final (unsealed)
   partition's bytes, discarding the original monolithic file's content in the same act — is itself a
   one-time, destructive, source-overwriting write, structurally identical in kind to the heal write
   the F-C3-P7-001 extension already governs: the moment it completes, the original monolithic
   content is irretrievably gone from the canonical file (recoverable only from git history), exactly
   the condition that extension's rationale cites. That rationale therefore applies identically here,
   not merely by analogy — leaving this ordinary completion of step (ii) unverified while requiring
   verification of its crash-recovery counterpart (the heal) was an unsanctioned asymmetry: a step
   (ii) write that silently truncates, partially flushes, or otherwise lands corrupted bytes on disk
   during a normal, uninterrupted migration run is undetectable at migration time and is discovered —
   if at all — only much later, after the source content is already gone (the exact silent-data-loss
   condition this BC's other destructive-write checks all exist to foreclose).

   Accordingly: after Postcondition 5 step (ii)'s `write_atomic` call returns, the backfill-split MUST
   perform a FRESH read-back of the canonical file from disk and verify its `(length, SHA-256)` equals
   `(final_bytes, final_sha256)` — the SAME two values already computed once, at split time, into the
   Backfill Recovery Manifest (Postcondition 3), and already durably published in step (i) of the SAME
   two-phase publish sequence, before this check ever runs — BEFORE the backfill-split reports success
   for that artifact. This is not a new value to compute: the Manifest already holds the exact oracle
   this check needs, derived from the same in-memory buffer that produced the bytes just written, so
   no additional hashing of the original content is required. If the read-back matches, the migration
   completes normally. If it does NOT match, the backfill-split MUST fail loud with **`E-SHD-013`**
   (NEW — added to `prd-supplements/error-taxonomy.md` in this SAME burst) and halt for operator
   investigation — the destructive write has already happened at that point (unlike checks (a)/(b)/(c)
   above, which gate BEFORE the original file is ever touched), so, exactly as the heal's own post-hoc
   check reasons, this final verification exists to surface the corruption immediately rather than let
   a silently-corrupted canonical file pass as successfully migrated.

   **Summary — the disk-read-back discipline now governs THREE destructive write sites, not one or
   two (this amendment, F-C3-P8-002): every sealed shard's write (the base clause (c) requirement,
   ruled F-C3-P6-002), the DANGEROUS-window heal write (F-C3-P7-001), and the happy-path
   canonical-truncate write (F-C3-P8-002, immediately above).** Each is independently verified via a
   fresh post-hoc disk read-back before the migration — or the interrupted-migration recovery — is
   allowed to report success. No destructive write this BC specifies is ever trusted on its own
   return value alone.
...
3. **The backfill-split is idempotent against a shard-index whose `backfill_manifest` field
   (Postcondition 3's schema-location correction) is POPULATED for that artifact — bare
   shard-index-FILE existence, or a non-empty `[[shard]]` array by itself, is NEVER sufficient
   evidence of a completed or in-progress backfill-split (this amendment, F-C3-P9-004, BLOCKING,
   superseding the prior file-existence-only reading `mechanism_a_backfill_already_migrated`
   shipped with).** An index may exist, with real sealed shards, purely because BC-1.18.006's
   ordinary per-write roll mechanism sealed it — the mandatory one-time backfill-split for that
   artifact may never have run at all (Postcondition 3's Composability clause, the roll-before-
   backfill ordering). `mechanism_a_backfill_already_migrated` MUST load the shard-index (not
   merely `stat()` the file) and test `index.backfill_manifest.is_some()`; a `false` result — index
   present, Manifest absent — routes to a genuine first-ever backfill run per Postcondition 3's
   Composability clause, NEVER to the idempotency disposition list below and NEVER to a fail-loud
   `E-SHD-011`/`E-SHD-012` disposition. **And the recovery-confirmation determination, once
   Manifest presence has been confirmed, MUST use Postcondition 5's exact whole-file `(length,
   SHA-256)` comparison against the Backfill Recovery Manifest — NEVER a structural byte-prefix
   comparison against a re-concatenation of already-sealed shards (this amendment, F-C3-P6-001).
   The Manifest is the sole authoritative basis this determination relies on because it AUTHORIZES
   AND VERIFIES the bytes a heal writes — it is NOT required to, and by Postcondition 3's own
   schema does NOT, STORE those bytes (this amendment, F-C3-P7-001).** `final_bytes` is a
   ...
   the write is issued. If a Manifest-confirmed partial or complete backfill-split has already run
   for an artifact (e.g., a prior interrupted attempt left a valid, fully-published shard-index
   AND Manifest), re-running the backfill MUST resolve to exactly one of three dispositions: (a) confirm the SAFE window
   ...
   **Residual `MissingBackfillManifest` disposition (this amendment, F-C3-P9-004) — what
   `E-SHD-011` form (b) legitimately covers going forward.** The idempotency redefinition above
   means the ordinary roll-before-backfill case (index present, Manifest absent) is no longer
   routed to a heal/recovery-confirmation check at all — it is a fresh backfill run
   (Postcondition 3's Composability clause), not an error. `MechanismABackfillError::
   MissingBackfillManifest` (`E-SHD-011` form (b)) therefore now fires ONLY in the genuinely
   residual case: `mechanism_a_backfill_already_migrated`'s upfront load confirms
   `backfill_manifest.is_some()`, routing execution into `heal_or_confirm_already_migrated`, but
   THAT function's own independent re-read of the same index file (Postcondition 5's
   Recovery-Confirmation Rule) then finds `backfill_manifest` absent — a genuine TOCTOU race (the
   index file was concurrently rewritten by a manifest-dropping operation between the two reads;
   structurally foreclosed going forward by Postcondition 3's schema-location correction for every
   IN-TREE write path, but not for an out-of-band hand edit or a legacy pre-amendment artifact) or
   on-disk corruption of the `backfill_manifest` TOML table specifically. This is the ONLY
   remaining legitimate citation for `E-SHD-011` form (b); `error-taxonomy.md`'s row for it MUST
   cite THIS clause, never Invariant 3's ordinary three-way disposition list above, which now
   presupposes Manifest presence throughout.
...
5. **(this amendment, F-C3-P8-002) No destructive write of this BC's one-time migration is ever
   trusted on its own return value alone — each receives an independent, fresh post-hoc disk
   read-back before the migration (or its interrupted-migration recovery) is allowed to report
   success.** This generalizes, as one explicit principle, what Invariant 4 already established for
   the narrower per-shard-cap case and what Postcondition 5's Manifest-Authoritative Slice-and-Verify
   Rule already established for the heal write (F-C3-P7-001): a sealed-shard write is verified
   against the in-memory partition buffer that was intended to be written (Postcondition 6(c),
   F-C3-P6-002); the DANGEROUS-window heal write is verified against the Backfill Recovery
   Manifest's `(final_bytes, final_sha256)` (Postcondition 6(c)'s F-C3-P7-001 extension); and the
   happy-path canonical-truncate write (Postcondition 5 step (ii)'s ordinary, non-recovery
   completion) is verified against that SAME Manifest pair (Postcondition 6(c)'s F-C3-P8-002
   extension). All three are destructive, one-time, source-overwriting writes for which an
   in-memory-only success signal cannot detect a silent truncation, partial flush, or other
   on-disk corruption before the source content it discards becomes unrecoverable except via git
   history — the exact condition this invariant, stated once for all three sites, exists to
   foreclose.
...
| EC-013 | **(this amendment, F-C3-P8-002)** The happy-path (non-recovery, first-ever, uninterrupted) canonical-truncate write (Postcondition 5 step (ii)) silently lands corrupted, truncated, or partially-flushed bytes on disk — e.g. a disk-full or power-loss condition mid-write that the underlying `write_atomic` call's own return value does not surface as an error | The mandatory post-hoc read-back (Postcondition 6(c)'s F-C3-P8-002 extension) detects the mismatch against the Backfill Recovery Manifest's `(final_bytes, final_sha256)` and fails loud with `E-SHD-013` (NEW) before the migration reports success — the operator is alerted at migration time, while the original monolithic file's content is still recoverable from git history, rather than discovering the corruption later via an undetected silent data-loss event. Distinct from EC-011/E-SHD-012: this fires on the ORDINARY, non-recovery completion path, not during a crash-recovery heal |
| EC-014 | **(this amendment, F-C3-P9-004)** A successful backfill-split (Manifest published) is followed by an ORDINARY BC-1.18.006 per-write roll against the artifact's fresh post-backfill "current" file (the roll's own trigger fires on a subsequent over-cap write) | The roll's `publish_shard_index_update` loads the existing `ShardIndex` (now carrying `backfill_manifest` as an ordinary struct field per Postcondition 3's schema-location correction), pushes its own new `[[shard]]` entry, and re-serializes the WHOLE struct — the Manifest field round-trips automatically, with no special-case code. A subsequent recovery-confirmation check against this artifact still finds `backfill_manifest.is_some()` and resolves correctly (typically SAFE, since the canonical file has since moved on past the backfill's own `final_bytes`/`final_sha256` — Postcondition 5's Recovery-Confirmation Rule already tolerates this via the AMBIGUOUS disposition should content diverge from both recorded pairs, which is expected and correct once further rolls occur, NOT a defect this EC concerns; this EC concerns ONLY whether the Manifest field itself survives the write, which it now unconditionally does) |
| EC-015 | **(this amendment, F-C3-P9-004)** BC-1.18.006's ordinary per-write roll seals content for one of the four mandatory backfill artifacts BEFORE the mandatory one-time backfill-split (Postcondition 1) has ever run for that artifact (the roll-before-backfill ordering — expected in practice, since these four artifacts are the ones most likely to trip the cap gate first) | `mechanism_a_backfill_already_migrated` loads the index and finds `backfill_manifest: None` despite a non-empty `[[shard]]` array — per the corrected Invariant 3, this is NOT "already migrated" and does NOT route to `heal_or_confirm_already_migrated` or any `E-SHD-011`/`E-SHD-012` disposition. The backfill-split proceeds as a genuine first-ever run: `original_bytes`/`original_sha256` are computed from the canonical file's CURRENT content (per Postcondition 3's Composability clause — the content the prior roll already sealed is preserved in its own pre-existing `[[shard]]` entries, outside this Manifest's scope); the backfill's own newly-sealed shards are appended with `seq` continuing from `existing_max_seq + 1`, never disturbing the pre-existing rolled entries; the published Manifest correctly accounts for only the canonical file's content as of this invocation |
...
| **(this amendment, F-C3-P8-002, EC-013)** `session-checkpoints.md`-shaped fixture: original content `A + A + "more\n"` (37 bytes), `shard_cap_bytes = 16` (`A = "## Checkpoint\nx\n"`, 16 bytes). Backfill runs for the FIRST time (uninterrupted, no crash, no prior shard-index): Postcondition 2 packing seals shard 1 = `A`; Postcondition 3 publishes the shard-index + Backfill Recovery Manifest (`original_bytes=37`, `original_sha256=sha256(A+A+"more\n")`, `final_bytes=21`, `final_sha256=sha256(A+"more\n")`) in step (i); Postcondition 5 step (ii) then `write_atomic`s the canonical file, but the underlying write is SIMULATED to land a truncated 18-byte result (`A + "mo"`) on disk instead of the intended 21-byte `A + "more\n"`, with `write_atomic`'s own return value reporting success | The mandatory post-hoc read-back (Postcondition 6(c)'s F-C3-P8-002 extension) reads the canonical file's actual on-disk bytes (18 bytes, `A + "mo"`) and computes `(18, sha256(A+"mo"))`, which does NOT equal `(final_bytes, final_sha256) = (21, sha256(A+"more\n"))` already recorded in the Manifest — the migration fails loud with `E-SHD-013` before reporting success. The shard-index and Manifest (already durably published in step (i)) remain intact for a subsequent recovery attempt, which finds the canonical file's `(18, hash)` matches NEITHER `original_bytes`/`original_sha256` (37 bytes) NOR `final_bytes`/`final_sha256` (21 bytes) and correctly reports the AMBIGUOUS `E-SHD-011` disposition on any later invocation, rather than silently accepting the 3-byte-truncated content as migrated | error |
| **(this amendment, F-C3-P9-004, EC-014)** `session-checkpoints.md`-shaped fixture backfilled ONCE (seals shard `seq=1`, publishes `[backfill_manifest]` with `original_bytes=37`/`final_bytes=21` per the EC-009 fixture), then a SEPARATE, ORDINARY `execute_roll`/`publish_shard_index_update` invocation (BC-1.18.006, simulating a later over-cap write against the fresh post-backfill current file) seals a NEW shard (`seq=2`) and re-publishes the shard-index | The re-published shard-index's `backfill_manifest` field still reads back `original_bytes=37`/`original_sha256=sha256(A+A+"more\n")`/`final_bytes=21`/`final_sha256=sha256(A+"more\n")` — byte-for-byte identical to what the backfill originally published — because `publish_shard_index_update` loaded, mutated, and re-serialized the WHOLE `ShardIndex` struct, of which `backfill_manifest` is now an ordinary field (Postcondition 3's schema-location correction). Pins the fix directly: pre-amendment, the side-channel `[backfill_manifest]` table would be silently absent from the re-serialized index, and a subsequent recovery-confirmation check would fail loud with `E-SHD-011` form (b) — `MissingBackfillManifest` — forever | edge-case |
| **(this amendment, F-C3-P9-004, EC-015)** Fresh artifact, no prior backfill. An ORDINARY `execute_roll` seals `seq=1` from live over-cap content, publishing a shard-index with `backfill_manifest: None` (correctly — no backfill has run). `run_mechanism_a_backfill_split` is THEN invoked for the first time against this SAME artifact, with the canonical file now holding whatever content has accumulated since the roll | `mechanism_a_backfill_already_migrated` loads the index, finds `backfill_manifest.is_none()` despite `shards.len() == 1`, and correctly reports NOT already migrated — routing to a fresh backfill run, never to `heal_or_confirm_already_migrated` and never to `E-SHD-011`/`E-SHD-012`. The backfill computes `original_bytes`/`original_sha256` from the canonical file's CURRENT content only (never attempting to reconstruct the pre-roll historical content, which remains correctly preserved in the pre-existing `seq=1` shard, outside this Manifest's scope); its own newly-sealed shards are appended starting at `seq=2` (continuing from the existing max), leaving the pre-existing `seq=1` rolled entry untouched; the published `backfill_manifest` accounts only for the canonical file's content as of this invocation. Pins the fix directly: pre-amendment, `mechanism_a_backfill_already_migrated`'s bare `fs::metadata(index_path).is_ok()` check would report `true` here, permanently short-circuiting the mandatory migration to either a silent no-op or (once routed through the heal path) a permanent `E-SHD-011` failure for exactly the artifacts this BC exists to fix | edge-case |
...
| VP-124 | **(this amendment, F-C3-P8-002)** Happy-path canonical-write verification invariant — Postcondition 5 step (ii)'s ORDINARY, non-recovery completion of the canonical-truncate write receives the SAME post-hoc disk read-back verification as a sealed-shard write and the DANGEROUS-window heal write: the written canonical file's `(length, SHA-256)` MUST match the Backfill Recovery Manifest's `(final_bytes, final_sha256)` before the migration reports success, and a mismatch fails loud (`E-SHD-013`, EC-013) rather than silently completing with corrupted content | integration test / fixture-based (the EC-013 CTV row above — a simulated truncated/corrupted happy-path write detected via read-back, `E-SHD-013` — plus a positive-control fixture confirming the check passes and reports success for a genuinely-correct write) |
| VP-124 | **(this amendment, F-C3-P9-004)** Manifest schema-location and order-independence invariant — the Backfill Recovery Manifest is an ordinary `Option<BackfillManifest>` field of `ShardIndex` itself (Postcondition 3's schema-location correction), never a side-channel table, and therefore survives EVERY subsequent `ShardIndex` load-modify-store cycle (BC-1.18.006's ongoing per-write roll, both self-heal reconciliation paths) with no special-case preservation code; idempotency ("already migrated") is determined SOLELY by `backfill_manifest.is_some()`, never by bare shard-index-file existence or a non-empty `[[shard]]` array alone — a manifest-less index with real sealed shards (the roll-before-backfill ordering) is correctly treated as NOT YET migrated, and the backfill-split tolerates and appends after such a pre-existing shard set rather than assuming a virgin index | integration test / fixture-based (EC-014: seal a backfill, THEN trigger an ordinary BC-1.18.006 roll against the resulting fresh current file, THEN assert `backfill_manifest` is still present and byte-identical; EC-015: seed a manifest-less shard-index via an ordinary roll BEFORE any backfill, invoke the backfill-split, assert it proceeds as a fresh run — never `AlreadyMigrated`, never `E-SHD-011`/`E-SHD-012` — and that its resulting shard-index has the pre-existing rolled entry UNCHANGED with the backfill's own entries appended at continuing `seq` numbers) |
...
**Fix-burst note (F-C3-P8-002, this amendment):** VP-124 gains a FIFTH facet (Happy-path
canonical-write verification invariant, Postcondition 5 step (ii) / Postcondition 6(c)'s new
F-C3-P8-002 extension, Invariant 5) — VP citation change; architect must propagate this facet to
`VP-INDEX.md`, `verification-architecture.md`, and `verification-coverage-matrix.md` per
`vp_index_is_vp_catalog_source_of_truth` (POLICY 9), landing alongside the still-outstanding third
facet (v1.6), fourth facet (v1.7), and the VP-123 third-facet propagation (v1.4), all still
outstanding as of this amendment. No new VP-NNN allocated — same additive-facet convention as the
prior four.

**Fix-burst note (F-C3-P9-004, this amendment, BLOCKING):** VP-124 gains a SIXTH facet (Manifest
schema-location and order-independence invariant, Postcondition 3's schema-location correction +
Composability clause, Invariant 3's redefined idempotency basis) — VP citation change; architect
must propagate this facet to `VP-INDEX.md`, `verification-architecture.md`, and
`verification-coverage-matrix.md` per `vp_index_is_vp_catalog_source_of_truth` (POLICY 9), landing
alongside the still-outstanding third facet (v1.6), fourth facet (v1.7), fifth facet (v1.8), and the
VP-123 third-facet propagation (v1.4), all still outstanding as of this amendment. No new VP-NNN
allocated — same additive-facet convention as the prior five.
...
- BC-1.18.013 — the governed one-time migration that ACTUALLY INVOKES this BC's algorithm against the live files via ADR-052's sanctioned execution path (`backfill-append-logs`); this BC specifies the per-file algorithm, BC-1.18.013 specifies the multi-file governed-activation wrapper (depended on by)
- BC-7.08.001 — the Cohort B fail-closed flip is gated on THIS BC completing (the existing oversized files must be split before flipping fail-closed, or the flip would immediately re-trigger the exact INDETERMINATE loop Layer 2 exists to eliminate); per BC-1.18.013 Postcondition 7, that completion is now achieved via BC-1.18.013's governed activation (depended on by)
...
|---------|------|--------|--------|
| 1.10 | 2026-09-25 | product-owner | Documentary-only amendment (Anchor-Back Rule; sibling burst to BC-1.18.013's creation, closing S-25.06's Spec-First Gate). Related BCs: added a reciprocal cross-reference to the NEW BC-1.18.013 ("Governed One-Time Migration for the Mechanism-A Backfill-Split of the Four Append-Log Files") — the governed-migration wrapper that actually invokes this BC's algorithm against the live files via ADR-052's sanctioned execution path (`backfill-append-logs`); reworded the existing BC-7.08.001 Related-BCs entry to note that this BC's completion (the Cohort-B gate condition) is now achieved via BC-1.18.013's governed activation, not a bare code-presence check. No Postcondition, Invariant, Edge Case, Canonical Test Vector, or VP content changed — this BC's own algorithm specification is untouched. input-hash recompute owed to state-manager. |
| 1.9 | 2026-09-10 | product-owner | Adjudication of finding BLOCKING-1 from a fresh-eyes PR reviewer on PR #831 (S-25.02 cluster-3), the FIRST finding on this BC raised by a PR-diff review rather than a LOCAL adversarial spec pass. **BLOCKING-1 — the Backfill Recovery Manifest does not survive an ordinary BC-1.18.006 roll, permanently stranding the mandatory one-time migration in a fail-loud state; AND the missing-manifest disposition's spec authority (v1.7/v1.8 Invariant 3's alleged sub-clause "(c)") was fabricated — no such sub-clause exists — RESOLVED.** Root cause, confirmed by direct inspection of `crates/factory-dispatcher/src/shard_manager.rs`: v1.6 (F-C3-P6-001) specified the Backfill Recovery Manifest as a hand-appended `[backfill_manifest]` TOML table living OUTSIDE the `ShardIndex` struct (a deliberate choice, per that function's own doc comment, to avoid a breaking struct-literal change at pre-existing test call sites) — but `publish_shard_index_update` (BC-1.18.006's ongoing per-write roll mechanism, merged and live in production via cluster-1/cluster-2, PRs #818/#824) loads a `ShardIndex`, mutates it, and re-serializes ONLY that struct's own fields, silently dropping the side-channel table on the very first roll after a backfill; both self-heal reconciliation paths (`self_heal_resume_from_truncate`, `self_heal_reconcile_missing_index_entries`) do the same. Two LIVE failure paths, both reachable in production ordering: (1) **roll-before-backfill** (the EXPECTED ordering — the four artifacts this BC exists to shrink are precisely the ones most likely to trip BC-1.18.005's cap gate first) — an ordinary roll creates a manifest-less index before the mandatory backfill ever runs; `mechanism_a_backfill_already_migrated`'s bare `fs::metadata(index_path).is_ok()` check then mis-reports "already migrated," the mandatory migration never executes for exactly the artifacts it exists to fix, and once routed through the heal path (`index.shards` non-empty) fails loud FOREVER with `MissingBackfillManifest`/`E-SHD-011` form (b). (2) **backfill-then-roll** — even a successful backfill's Manifest is destroyed by the very next ordinary roll, after which Invariant 3's promised idempotent SAFE re-check instead returns `MissingBackfillManifest` permanently. Separately confirmed by direct inspection of this file's own v1.7/v1.8 Invariant 3 text: `error-taxonomy.md`'s `E-SHD-011` row cited "BC-1.18.008 v1.7 Invariant 3(c)'s explicit sanction of reusing `E-SHD-011` for the missing-manifest case" as form (b)'s spec authority — Invariant 3 has NO sub-clause (c) addressing a shard-index-without-manifest state at all; its three-way disposition list (a)/(b)/(c) covers only SAFE/DANGEROUS/AMBIGUOUS determinations that ALREADY PRESUPPOSE a Manifest exists to compare against. The shipped code therefore implemented a disposition with fabricated spec authority. **ADJUDICATED (design decision, evaluated and refined from the reviewer's own suggested fix): promote the Backfill Recovery Manifest to `backfill_manifest: Option<BackfillManifest>`, an ORDINARY ADDITIVE FIELD of the shared `ShardIndex` struct itself** (`#[serde(default, skip_serializing_if = "Option::is_none")]`), the EXACT precedent BC-1.18.007's `retention_count` already set for a sibling BC extending this same shared struct — REJECTED as the root cause the v1.6 rationale that a new field would force breaking test-call-site changes (an OPTIONAL field imposes no such requirement). This closes failure path (2) STRUCTURALLY: every `ShardIndex`-touching write site (BC-1.18.006's roll, both self-heal paths) now carries the Manifest forward automatically as an ordinary consequence of struct serialization, with ZERO special-case preservation code at any of those sites. Retires the side-channel `BackfillManifestWrapper`/`BackfillManifestDocument`/`read_backfill_manifest` helpers. Closing failure path (2) alone was insufficient, since an index can ALSO exist with real sealed shards and no Manifest purely because it was NEVER backfilled (failure path (1)) — so ALSO **redefined Invariant 3's idempotency basis from bare shard-index-file existence to Manifest presence** (`backfill_manifest.is_some()`, never mere `shards`-non-empty or file-`stat()`-success), and **added a Composability clause to Postcondition 3** requiring the backfill-split, when it finds a manifest-less-but-shard-bearing index (the roll-before-backfill case), to proceed as a genuine first-ever run rather than short-circuiting or failing loud: `original_bytes`/`original_sha256` are always computed from the canonical file's CURRENT content at invocation time (never a reconstructed full-historical-file notion — content a prior roll already sealed stays in ITS OWN pre-existing `[[shard]]` entries, outside the Manifest's scope), and the backfill's own newly-sealed shards are APPENDED with `seq` continuing from `existing_max_seq + 1`, never disturbing the pre-existing rolled entries. Added a **Residual `MissingBackfillManifest` disposition** clause to Invariant 3 stating precisely what `E-SHD-011` form (b) legitimately covers post-amendment: ONLY a genuine TOCTOU race between `mechanism_a_backfill_already_migrated`'s upfront Manifest-presence check and `heal_or_confirm_already_migrated`'s own independent re-read, or on-disk corruption of the `backfill_manifest` table specifically — never the ordinary roll-before-backfill case, which this amendment now correctly routes to a fresh backfill run instead. Added NEW EC-014 (Manifest survives an intervening BC-1.18.006 roll, pinning the struct-field fix) and NEW EC-015 (roll-before-backfill ordering — backfill tolerates and appends after a pre-existing manifest-less shard set) with matching Canonical Test Vectors. Added VP-124's SIXTH facet (Manifest schema-location and order-independence invariant) — VP citation change routed to architect per `vp_index_is_vp_catalog_source_of_truth` (POLICY 9), to land alongside the still-outstanding VP-123 third-facet (v1.4), VP-124 third-facet (v1.6), fourth-facet (v1.7), and fifth-facet (v1.8) propagations. **Implementer scope (routed, NOT yet executed by this burst — do NOT write Rust code as part of this spec amendment):** (1) promote `BackfillManifest` to a `backfill_manifest: Option<BackfillManifest>` field on `ShardIndex`, `#[serde(default, skip_serializing_if = "Option::is_none")]`; delete the now-redundant `BackfillManifestWrapper`/`BackfillManifestDocument` structs and `read_backfill_manifest`'s standalone re-parse, replacing every call site with `load_shard_index(...).backfill_manifest`; (2) rewrite `mechanism_a_backfill_already_migrated` to load the index (not merely `stat()` the file) and return `index.map(\|i\| i.backfill_manifest.is_some()).unwrap_or(false)`; (3) `run_mechanism_a_backfill_split` must handle the "index present, Manifest absent" case (roll-before-backfill) as a fresh run: compute `original_bytes`/`original_sha256` from the canonical file's current on-disk content, and append newly-sealed `[[shard]]` entries starting at `existing_max_seq + 1` rather than assuming a virgin (empty `shards`) index; (4) add TWO new regression tests per the PR reviewer's explicit request — a roll-then-backfill test (asserts the backfill runs to completion and correctly interleaves `seq` numbering against a pre-existing rolled shard) and a backfill-then-roll-then-backfill test (asserts the Manifest survives the intervening roll and the second backfill invocation correctly reports `AlreadyMigrated` via the SAFE disposition). **Stories affected by BC changes (→ story-writer, per `bc_array_changes_propagate_to_body_and_acs`):** S-25.02 — no `bcs:` frontmatter array change (already listed); AC-013/AC-014 body text requires propagation of the schema-location correction, the Composability clause, and the redefined Invariant 3 idempotency basis — routed to story-writer (or completed directly by product-owner in the same burst if mechanical; see this burst's own routing note). **`prd-supplements/error-taxonomy.md` amended in the SAME burst** (v1.13→v1.14: `E-SHD-011` form (b)'s citation corrected from the fabricated "Invariant 3(c)" to the real "Invariant 3's Residual `MissingBackfillManifest` Disposition clause (F-C3-P9-004)"; `E-SHD-002`'s row extended to document the PR's new THIRD real emission, `ShardRetentionError::ArchivalIndexEntryVanished`; exhaustiveness attestation count bumped) — not deferred. **Input-hash recompute owed to state-manager** (this file's own content changed v1.8→v1.9). |
| 1.8 | 2026-09-10 | product-owner | Adjudication of finding F-C3-P8-002 (MEDIUM) from a fresh LOCAL adversarial pass on S-25.02 F4 cluster-3 (the pass immediately following the v1.7 burst). **F-C3-P8-002 — happy-path canonical-truncate write lacks the post-hoc read-back the heal write requires (asymmetry + latent silent-data-loss), RESOLVED.** Postcondition 6(c)/Invariant 4 already mandate post-hoc disk read-back for every sealed-shard write, and the F-C3-P7-001 extension already extends that requirement to the DANGEROUS-window heal write, on the rationale that the heal write "permanently replaces the canonical file's content at the moment the pre-heal content is irretrievably gone." That rationale applies IDENTICALLY to the happy-path canonical-truncate write in Postcondition 5 step (ii) (the final `write_atomic` that discards the original monolithic file — the final partition's bytes exist ONLY in that canonical file once written) — but the spec did not textually enumerate the happy-path write, so a spec-literal implementation would not read it back, leaving a silent truncation there as permanent data loss with zero migration-time detection: the more-frequently-executed path (every uninterrupted migration run) was LESS verified than its own crash-recovery counterpart. ADJUDICATED: added an **Extension to the happy-path canonical-truncate write** paragraph to Postcondition 6(c) (immediately after the F-C3-P7-001 heal extension), requiring the SAME fresh post-hoc disk read-back — verified against the Backfill Recovery Manifest's `(final_bytes, final_sha256)`, already durably published in Postcondition 5 step (i) before step (ii) runs, so no new value needs computing — after step (ii)'s `write_atomic` call returns and before the migration reports success; a mismatch fails loud with **NEW `E-SHD-013`** (added to `prd-supplements/error-taxonomy.md` in this SAME burst) rather than reporting success over corrupted content. Added a **Summary** paragraph to Postcondition 6(c) naming all three destructive-write sites (sealed shard / heal / happy-path canonical) now under the same discipline, and a new **Invariant 5** stating the generalized principle once (no destructive one-time-migration write is ever trusted on its own return value alone). Added EC-013 (happy-path write silently corrupted on disk, detected via read-back, fails loud `E-SHD-013`) and a matching Canonical Test Vector (a simulated truncated happy-path write on the `A + A + "more\n"` fixture, caught by the read-back, with a follow-on note that a subsequent recovery attempt against the resulting on-disk state correctly reports the AMBIGUOUS `E-SHD-011` disposition rather than silently accepting the corruption). Added VP-124's FIFTH facet (Happy-path canonical-write verification invariant) — VP citation change routed to architect per `vp_index_is_vp_catalog_source_of_truth` (POLICY 9), to land alongside the still-outstanding VP-123 third-facet (v1.4), VP-124 third-facet (v1.6), and VP-124 fourth-facet (v1.7) propagations. **Implementer scope (routed, not yet executed by this burst):** `run_mechanism_a_backfill_split`'s happy-path completion of the canonical-truncate write must gain a fresh disk read-back immediately after `write_atomic` returns, comparing `(length, sha256)` against the shard-index's already-published `[backfill_manifest]` `(final_bytes, final_sha256)`, and must surface the NEW `E-SHD-013` variant on mismatch (a new `MechanismABackfillError` variant, mirroring `SliceVerificationFailed`'s shape but scoped to the non-recovery path) without writing anything further. **Stories affected by BC changes (→ story-writer, per `bc_array_changes_propagate_to_body_and_acs`):** S-25.02 — no `bcs:` frontmatter array change (already listed); story-writer should confirm the story body's PC5/PC6/Invariant summaries reflect this amendment's happy-path read-back requirement. **`prd-supplements/error-taxonomy.md` amended in the SAME burst** (v1.11→v1.12: `E-SHD-013` added NEW; `E-SHD-012`'s Message Format cell also CORRECTED to the code's actual `{detail}`-placeholder form per the sibling finding F-C3-P8-001, and `E-SHD-011`'s Message Format cell completed to document its second real emission — see that file's own v1.12 changelog entry for full detail) — not deferred. **Input-hash recompute owed to state-manager** (this file's own content changed v1.7→v1.8). |
