...
| AC-006 (bounded 0-byte-destination reclaim, EC-025) | Unlike the NON-EMPTY case, a 0-byte file already occupying the next-expected seq path is structurally NEVER genuine sealed history (this BC's own write paths only ever seal non-empty content) — `publish_sealed_shard` reclaims it: SEC-001 `lstat()`s (`symlink_metadata` — never dereferences a symlink at the destination) the collision EXACTLY ONCE to confirm it is a regular, 0-byte file (never a symlink); FIX-MED-1 then re-verifies that same identity via an ALREADY-OPEN file handle's own metadata IMMEDIATELY BEFORE unlinking, closing the TOCTOU window between the `lstat()` probe and the `remove_file` call (a legitimate concurrent writer replacing the placeholder with real content in that window aborts the reclaim rather than being silently discarded); the retry's content is then STAGED FIRST at a fresh, collision-resistant random-nonce temp path — never touching the destination at all — and only AFTER that staging succeeds does it unlink the reclaimable 0-byte destination and publish the staged content onto that now-vacated path via a hard-link (the retry never re-invokes `write_exclusive` itself, since it already staged its own content directly), so a staging failure (e.g. a colliding temp path) leaves the reclaimable destination COMPLETELY UNTOUCHED rather than destroyed with nothing published in its place. The roll then SUCCEEDS: the reclaimed seq (1) is reused (never skipped past), the real 50,000-byte pre-roll content is durably sealed at that exact path, and the shard-index advances by exactly one entry — never `E-SHD-009`. | `AC-006-0-byte-reclaim.{gif,webm}` | `test_BC_1_18_006_EC025_FC2P8_002_execute_roll_reclaims_zero_byte_destination_and_succeeds` (`shard_manager.rs` unit test, drives the full `execute_roll` staged sequence directly) | PASS (`Ok(Some(_))`; reclaimed seq == 1; sealed content == pre-roll bytes; index has exactly 1 entry; canonical == 0 bytes) |
