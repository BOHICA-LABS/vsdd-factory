...
## D-1236 — S-25.02 cluster-5 F4 stub-architect ambiguity adjudication (3 rulings) + ADR-052 v1.16

**Date:** 2026-09-22
**Burst type:** architect-adjudication persist (state-manager, single-commit TD-VSDD-053)
**Basis:** stub-architect's cluster-5 F4 Red Gate stub commit (`feature/S-25.02-b2-sharding` @ `adbc795a`, 48 `todo!()` functions, T-10/T-11 surface, crate green, baseline 3204 tests pass) surfaced 3 ambiguities in BC-1.18.010/BC-1.18.011/ADR-052 that stub-architect could not adjudicate itself (spec-content adjudication is architect-owned per CLAUDE.md's Agent Routing Table); architect resolved all 3 this burst; this burst persists the architect's already-edited ADR-052 body (v1.15→v1.16, uncommitted in the `.factory/` worktree prior to this burst) plus the codification.

**Action:** Architect adjudicated 3 cluster-5 F4 stub-ambiguity questions:

1. **Native-gate precedence CORRECTED (defect, not spec gap).** `bc_index_migration_admission_precheck` MUST run BEFORE `shard_cap_precheck` for BC-INDEX-path dispatches, and `shard_cap_precheck` MUST be SKIPPED entirely (not merely have its result discarded) when migration-admission blocks the call. The stub surface's `.or()`-after-both-computed composition would have let `execute_roll`'s destructive side effect run before the migration-admission block could fire — a genuine defect, not an open design question. BC-1.18.011 Precondition 6(b) + ADR-052 §Decision 5a jointly entail this ordering; no BC or ADR text amendment required — the fix is in the T-10/T-11 implementation surface (executor.rs precheck-composition order), owed to implementer during cluster-5 TDD.
2. **E-MAINTENANCE-001 → `HookResult::Block` CONFIRMED.** Cross-checked against error-taxonomy.md's E-MAINTENANCE-001 entry, hook-sdk `result.rs`'s `HookResult` variant set, and the existing `E-SHD-*` family's `Block` precedent (BC-1.18.006/010). No ambiguity once cross-referenced; no edit required to any artifact.
3. **§Decision 5c Bash full-command classifier + its 4 dispatcher-guard amendments (§Decision 5b), and §Decision 11 executable-binding, are OUT of cluster-5 T-10/T-11 TDD scope.** These are `[D-1232-OBL-4]` (devops-engineer, F4 activation-boundary deliverable per the D-1232 ratification), not stub-architect/test-writer/implementer TDD-scope work. ADR-052 §Decision 5c carried a stale "(cluster-5 TDD scope)" label on its negative-tests list, predating the D-1232 OBL-4 split and contradicting the ADR's own §Status block + §Files-to-Change table (both of which already correctly scope `validate-factory-path-staging`/`validate-factory-path-staged` to devops-engineer/activation). Left uncorrected, this stale label risked test-writer authoring Bash-admission Red Gate tests inside cluster-5's T-10/T-11 TDD scope for logic `bc_index_migration_admission_precheck` (Edit/Write/MultiEdit admission only) does not and should not implement.

ADR-052 v1.15→v1.16 fixes ruling (3): §Decision 5c's stale "Negative tests (cluster-5 TDD scope)" label corrected to attribute the negative-test list to devops-engineer's OBL-4 activation deliverable, cross-referenced against §Status + §Files-to-Change; v1.16 changelog row added. NO §Decision 1–11 substance change, NO concurrency-state-machine change (the §Decision 4e/5a/7c freeze from D-1231/D-1233 is unaffected). Rulings (1) and (2) require no spec-artifact edit — (1) routes to implementer as an in-scope T-10/T-11 fix (not a spec amendment, since BC-1.18.011 PC6(b) + ADR-052 §5a already entail the correct ordering); (2) required no artifact change at all.

The architect's ADR-052 body edit (frontmatter `version: "1.15"→"1.16"`, §Decision 5c label correction, v1.16 changelog row) was already made and left uncommitted in the `.factory/` worktree prior to this burst. This burst's job was exclusively: (1) ARCH-INDEX propagation (row-body amendment segment appended + frontmatter version/last_amended/changelog per BC-5.45.001 write-path discipline), (2) ADR-052 input-hash verification, (3) this decision-log codification, and (4) the STATE.md advance + atomic commit — state-manager performed no ADR-052 body edit (ADR-052 content is architect-owned per CLAUDE.md's Agent Routing Table).

**Input-hash:** ADR-052 `input-hash: "a9d309a"` — `compute-input-hash --check` CLEAN (exit 0, no drift) both before and after this burst. `--update` confirms "already current (a9d309a)" — the ADR's `input-hash` tracks its declared `inputs:` (upstream generative files), not its own body content; a body-only edit (status/version/§5c label/changelog) does not change the input-hash, consistent with every prior ADR-052 fix-burst in this cascade (D-1228..D-1235 (exhaustive)).

**POLICY 18 applicability:** N/A — ADR-052 is not a per-story artifact; POLICY 18's three-way input-hash parity gate governs `S-NN.NN` stories only.

**Sidecar note:** this burst's commit also folds in pre-existing uncommitted `sidecar-learning.md` + `regression-state.json` deltas (session-end telemetry lines; a `cargo test -p factory-dispatcher` regression-gate run from the `S-25.02-b2-sharding` worktree, `status: pass`) to leave the `.factory/` worktree clean at commit time — incidental inclusions, not this burst's own content.

**STATE.md this same burst:** `pipeline:` stays `in_progress` (unchanged — no pause/resume transition this burst). Phase Progress row appended: `S2502-CLUSTER5-F4-TDD-STUBS-ADJUDICATION 2026-09-22 (D-1236)`. Session Resume Checkpoint refreshed: §4 item #4 now ACTIVE/IN PROGRESS with sub-state "cluster-5 F4 TDD: stubs committed (`feature/S-25.02-b2-sharding` @ `adbc795a`, worktree `.worktrees/S-25.02-b2-sharding`), architect rulings D-1236 applied, test-writer authoring failing tests next"; §5 WIP branches now lists `feature/S-25.02-b2-sharding`. STATE.md v10.67→v10.68.

**BC-INDEX/VP-INDEX/STORY-INDEX/error-taxonomy/prd.md:** all UNCHANGED this burst (no BC/VP/story/error-taxonomy content touched by any of the 3 rulings).

**OWED (unchanged):** OWED #2 (907-file `compute-input-hash --scan --update` sweep). [D-1232-OBL-1..4] all remain OPEN/PENDING, cluster-5-scoped, UNCHANGED by this burst (ruling 3 clarifies OBL-4's scope boundary; it does not close or modify the obligation itself).

### Canonical 6-column row (STATE.md Decisions Log)

| D-1236 | D-1236-S2502-CLUSTER5-F4-STUB-AMBIGUITY-ADJUDICATION | (see prior entry above) | S-25.02 F4 | 2026-09-22 |

## D-1237 — S-25.02 cluster-5 B2 second-level sub-shard chunk-boundary algorithm spec-closure (ADR-051 v1.16 §Decision 18, BC-1.18.010 v1.10, BC-1.18.011 v1.9, VP-142, S-25.02 v4.5)

**Date:** 2026-09-22
**Burst type:** multi-specialist spec-closure persist (state-manager, single-commit TD-VSDD-053)
**Basis:** during cluster-5 F4 TDD, the implementer surfaced (via an inline flag in `run_bc_index_migration`) that the T-10/T-11 stub/implementation surface performed only first-level splitting — one file per `### SS-NN` section, with `sub_sharded`/`sub_manifest` hardcoded `false`/`None` — even though SS-05 (~88,695 bytes) and SS-06 (~85,407 bytes) already exceed the provisional 48KiB (49,152-byte) cap and require second-level sub-sharding per BC-1.18.011 Postcondition 6. Neither ADR-051 nor BC-1.18.010/BC-1.18.011 had ever specified the actual chunk-boundary MECHANISM (where inside a subsystem's body a sub-shard boundary falls) — Decision 7/8 specified the addressing END-STATE and Decision 10/BC-1.18.011 PC6 specified that the migration must cover the sub-split, but not the algorithm. This is a genuine spec gap, not an implementation defect: shipping the unmodified migration against live data would produce a directly falsifiable BC-1.18.011 PC6 violation. Human approved the architect's assess-first proposal (`b2-subshard-algorithm-proposal.md`, 2026-09-22) to close the gap via a new ADR-051 addendum before test-writer/implementer proceed.

**Action:** Three specialist agents authored content, each already edited and left uncommitted in the `.factory/` worktree prior to this burst; this burst's job was exclusively propagation, input-hash reconciliation, count-propagation reconciliation, codification, and the atomic commit:

1. **Architect — ADR-051 v1.15→v1.16, NEW §Decision 18** ("Second-Level Sub-Shard Chunk-Boundary Algorithm for B2 SS-05/SS-06 [and Future Over-Cap Subsystems] Second-Level Sub-Sharding"). Key content: (1) trigger reuses §Decision 2's SAME `shard_cap_bytes` formula unmodified, no new formula — a sub-shard file is governed by the same byte-`stat()` trigger as any first-level `"flat"` artifact; (2) input reuses `extract_and_sort_bc_rows` (already implemented for content-preservation/census per §Decision 10) — chunk-boundary order is, by construction, the SAME canonical order the content-preservation SHA-256 and independent census already key on; (3) a fixed preamble (`### SS-NN` heading + table header/separator rows) is replicated verbatim into EVERY sub-shard, never packed with only the first, and its byte cost is counted against `shard_cap_bytes` as that sub-shard's starting `current_bytes`; (4) `chunk_subsystem_rows_into_sub_shards(sorted_rows: &[(BcId, String)], preamble: &str, shard_cap_bytes: u64) -> Vec<SubShardChunk>` — a pure, canonical-BC-ID-sorted, greedy-pack-until-cap, single left-to-right pass, never a balanced/rebalanced partition; (5) determinism argument — identical input bytes in identical order always produce identical boundaries, on any invocation/machine/retry, since the function is pure over its three inputs; (6) BC-ID-range sub-manifest schema (`shards/BC-INDEX-SS-05.manifest.toml`, `[[sub_shard]]` TOML entries with `range_start`/`range_end`), reusing the already-implemented-but-previously-unused reader (`resolve_bc_shard_path`/`SubShardManifest`/`SubShardRangeEntry`/`load_sub_shard_manifest`, Decision 8); (7) edge-case rulings table — lone-oversized-row (allowed as an over-cap lone sub-shard, `tracing::warn!`, never fail-loud, bounded by BC-1.18.005 PC6's `MAX_SINGLE_RECORD_BYTES` margin), exactly-at-cap (`<=` inclusive, matching BC-1.18.005 PC3's convention), sub-shard letter exhaustion (base-26 `.aa`/`.ab`... extension via `sub_shard_letter`, never fail-loud), independent census (already covered by existing PC2/`verify_independent_census`, zero new logic), third-level splitting (out of scope, per BC-1.18.010's own Canonical Test Vectors); (8) steady-state unification — item 7 (renumbered from the ADR's own internal numbering; see the ADR body for the full sequence) states the SAME pure function is called as a FULL REBUILD (never incremental append-and-split) for the future per-write steady-state trigger, guaranteeing migration-time and steady-state produce byte-identical boundaries for the same row set; (9) flags a SEPARATE, NOT-closed-here gap — BC-1.18.010 Precondition 3's steady-state-trigger "per-subsystem body table" artifact-shape extension to BC-1.18.005/BC-1.18.006 does not yet exist (see Drift Item below); (10) confirms zero ADR-052 concurrency-core impact — pure chunking at the staging-content-generation phase, reusing the existing `pending_canonical_moves`/`migration_durable_write`/intent-log machinery unmodified, zero new txn states, zero new crash-recovery cases. Not a POLICY 22 reversal — a net-new addendum supplying a mechanism Decision 10 assumed but never specified, mirroring Decisions 15/16/17's own precedent for closing an ADR-level specification gap.
2. **Product-owner — BC-1.18.010 v1.9→v1.10.** Postcondition 4 amended: replaced the worked-example-only "growth-based (e.g., ... covering `BC-5.01.001`..`BC-5.30.099`)" boundary text with the actual deterministic chunk-boundary rule per ADR-051 §Decision 18 (canonical-BC-ID-sorted, greedy-pack-until-cap, single left-to-right pass, reusing the SAME `shard_cap_bytes` as first-level splitting per Invariant 4, fixed preamble replicated verbatim into every sub-shard and counted against cap). VP Anchors section gained a NEW VP-142 cross-reference. No change to Postcondition 4's SS-05/SS-06 measured-size facts or migration-coverage claim (still covered by BC-1.18.011).
3. **Product-owner — BC-1.18.011 v1.8→v1.9.** Postcondition 6 amended: named the concrete `chunk_subsystem_rows_into_sub_shards(sorted_rows, preamble, shard_cap_bytes) -> Vec<SubShardChunk>` function contract the migration invokes, and added the 4 explicit migration-time edge-case rulings as inline prose bullets (lone-oversized-row, exactly-at-cap, sub-shard letter exhaustion, each-row-in-exactly-one-sub-shard via existing PC2 census). Added a `## Verification Properties` table row and `## VP Anchors` entry for NEW VP-142 (proptest; hosted on THIS BC's Postcondition 6), cross-referenced from BC-1.18.010 Postcondition 4. No change to Postconditions 1-5, 7-8 or to the existing content-preservation/census/atomicity/rollback machinery.
4. **Formal-verifier — NEW VP-142** ("B2 Second-Level Sub-Shard Chunk-Boundary Determinism and Correctness"): proptest over synthetic canonical-BC-ID-sorted row sets + preamble + cap — determinism, exhaustive row coverage, cap-inclusive boundary semantics, lone-oversized-row edge case, letter-exhaustion edge case, range non-overlap-and-coverage. `source_bc: BC-1.18.011 §PC6(NEW, ADR-051 §Decision 18)`. Hosted on BC-1.18.011, cross-referenced from BC-1.18.010 PC4. Already present in VP-INDEX.md body (Full Index row + Traceability row + Summary table `total_vps: 142`) prior to this burst; this burst closed the frontmatter `version:`/`last_amended`/`changelog:` gap (v3.22→v3.23) per BC-5.45.001 write-path discipline.
5. **Story-writer — S-25.02 v4.4→v4.5.** AC-017 EXTENDED IN PLACE (heading unchanged) — added a deterministic sub-shard chunk-boundary rule paragraph + VP-142 cross-reference. AC-018 EXTENDED IN PLACE (postcondition 6 trace citation added) — added a sub-shard chunking implementation obligation paragraph naming the concrete function contract and the 4 edge-case rulings + VP-142 cross-reference; explicit decision recorded that NO new EC-NNN rows were added (the edge-case rulings live as inline prose in BC-1.18.011 PC6, not as `## Edge Cases` table rows, so mirroring them as synthetic EC-NNN would break the story's 1:1 source-traceability precedent). Both ACs EXTENDED IN PLACE — NOT new ACs; RED-Gate/stub-coverage count stays 25 ACs. `verification_properties:` frontmatter: VP-116..VP-141 → VP-116..VP-142 (26→27 VPs). §Behavioral Contracts table: BC-1.18.010 cell v1.9→v1.10, BC-1.18.011 cell v1.8→v1.9. §Token Budget: BC-1.18.010 3,600→4,300, BC-1.18.011 4,200→5,100, VP line 6,000→6,200, Story spec 12,600→13,500, Total ~92,800→~95,500, Budget usage ~46%→~48%. `## Tasks` T-10/T-11 extended.

**Propagation this burst (state-manager):**

- **BC-INDEX.md v5.96→v5.97** — BC-1.18.010 catalog cell appended `\| v1.10 (...)`; BC-1.18.011 catalog cell appended `\| v1.9 (...)`; frontmatter `version:`/`last_amended:`/`changelog:` bumped per BC-5.45.001. `total_bcs` UNCHANGED 2006.
- **VP-INDEX.md v3.22→v3.23** — frontmatter `version:`/`last_amended:`/`changelog:` bumped; `total_vps: 142` already correct in-body (architect/formal-verifier), this burst closed the frontmatter gap.
- **verification-architecture.md v1.35→v1.36 + verification-coverage-matrix.md v1.33→v1.34** — already edited by the architect (POLICY 9 propagation: SS-01 module-table row for VP-142, SS-01 subtotal P 7→8, Grand Total P 11→12, Total 141→142) and already recomputed to `input-hash: "c54ef1c"`; this burst verified `compute-input-hash --check` CLEAN on both (confirmed, not recomputed).
- **STORY-INDEX.md v4.474→v4.475** — catalog-row BC bracket `BC-1.18.010 v1.9→v1.10, BC-1.18.011 v1.8→v1.9`; catalog-row + E-25 blockquote (Input-hashes list + POLICY 18 sentence) input-hash re-synced `171c3bb`→`9b4fd49`; frontmatter `version:`/`last_amended:`/`changelog:` bumped.
- **ARCH-INDEX.md v4.44→v4.45** — ADR-051 row-body addendum segment appended (v1.15→v1.16, §Decision 18 summary); frontmatter `version:`/`last_amended:`/`changelog:` bumped; `total_adrs` UNCHANGED 52.

**Input-hash recomputes** (`plugins/vsdd-factory/bin/compute-input-hash <path> --update`, run from repo root against `.factory/`-relative paths): BC-1.18.010.md `6e2d3a3`→`6b1de10`; BC-1.18.011.md `e027627`→`e823637`; S-25.02 story `171c3bb`→`9b4fd49`. Follow-up `--check` CLEAN (exit 0) on all three, plus verification-architecture.md and verification-coverage-matrix.md (confirmed unchanged at `c54ef1c`). ADR-051.md and VP-INDEX.md carry no `input-hash:` field (architect/VP-INDEX conventions — not applicable).

**POLICY 18 three-way parity:** S-25.02 frontmatter `input-hash: "9b4fd49"` = STORY-INDEX catalog-row cite `9b4fd49` = E-25 blockquote cite `9b4fd49`. VERIFIED (`validate-cross-site-correspondence` re-run clean after the sync edits).

**Count-propagation reconciliation (S-7.02 defensive sweep):** `validate-count-propagation` fired mid-burst on STATE.md's stale `## Identifier Conventions` Verification Property row (still citing VP-INDEX v3.21/`141`/`26 VPs` from D-1225-era text, 12 bursts stale). Corrected to VP-INDEX v3.23/`142`/`27 VPs` (`VP-116..142`). Swept: STATE.md (fixed), VP-INDEX.md (source of truth, already 142), BC-INDEX.md/ARCH-INDEX.md/STORY-INDEX.md (no VP-count citations in those files' own frontmatter/body requiring change this burst). No other file found still citing the old `141`/`26 VPs` figures as a LIVE (non-historical) citation — remaining `141`/`26 VPs` occurrences elsewhere in STATE.md (Blocking Issues rows `[D-1174..]`, `[S2502-CLUSTER1-PASS*]`, Drift Items `[D-1186]` VP-count-discrepancy rows) are frozen historical-record prose describing past burst states and are correctly left unchanged (POLICY 1 append-only; rewriting historical rows would falsify the record).

**Drift Item [D-1237-DRIFT-001] (human-authorized deferral):** BC-1.18.010 Precondition 3 references a "per-subsystem body table" artifact-shape extension to BC-1.18.005/BC-1.18.006 for the STEADY-STATE growth trigger (the ongoing per-write case, as opposed to this one-time migration) that does not yet exist in either BC. HUMAN-AUTHORIZED deferral (2026-09-22): this is a distinct future behavior — it only fires when a NEW subsystem later grows over cap after migration (SS-05/SS-06 themselves are fully handled by this one-time migration, closing the immediate falsifiable-violation risk). Concrete dependency: future-subsystem growth over cap. Anchor: a future E-25 backlog follow-up story (to be authored alongside S-25.03/S-25.05/S-25.06) — NOT "later"/"next cycle" as a bare placeholder. Satisfies CLAUDE.md Canonical Principle Rule 3 (explicit human direction + concrete future dependency + specific future story/wave anchor). NOT lost — recorded in STATE.md `## Drift Items / Tech Debt`.

**STATE.md this same burst:** `pipeline:` stays `in_progress` (unchanged — no pause/resume transition). Phase Progress row appended: `S2502-CLUSTER5-SUBSHARD-SPEC-CLOSURE 2026-09-22 (D-1237)`. Current Phase Steps row appended (CPS window held at 11 rows, continuing the prior deferral pending issue #841 fix confirmation). Session Resume Checkpoint refreshed: §4 REMAINING-WORK item #4 sub-state advanced — "B2 sub-shard chunk-boundary spec-closure DONE (D-1237); NEXT = test-writer authors 7 Red-Gate tests against the T-10/T-11 stub surface (extended for the sub-shard chunking behavior), then implementer (subject to `[D-1232-OBL-1]` impl-phase Kani obligation before TDD completion)." §5 WIP branches UNCHANGED (`feature/S-25.02-b2-sharding` @ `adbc795a` remains the live branch — this burst is spec-only, no code-branch advance). trajectory-tail →1→1→2→1 LENGTH=4 UNCHANGED (no new adversary pass ran this burst). STATE.md v10.68→v10.69.

**Sidecar note:** this burst's commit also folds in pre-existing uncommitted `sidecar-learning.md` + `regression-state.json` deltas (session-end telemetry lines) to leave the `.factory/` worktree clean at commit time — incidental inclusions, not this burst's own content.

**OWED (unchanged):** OWED #2 (907-file `compute-input-hash --scan --update` sweep). `[D-1232-OBL-1..4]` all remain OPEN/PENDING, cluster-5-scoped, UNCHANGED by this burst — this burst is pure spec-closure content with zero ADR-052 concurrency-core impact (confirmed explicitly by ADR-051 §Decision 18 item 9/10), so none of the 4 obligations are touched, advanced, or clarified by it (contrast D-1236, where ruling 3 clarified OBL-4's scope boundary).

### Canonical 6-column row (STATE.md Decisions Log)

| D-1237 | D-1237-S2502-CLUSTER5-B2-SUBSHARD-SPEC-CLOSURE | (see STATE.md Decisions Log table for the full canonical row text — identical content, single source written once at the STATE.md burst per TD-VSDD-053 single-commit discipline) | S-25.02 F4 | 2026-09-22 | **S-25.02 cluster-5 F4 stub-architect ambiguity adjudication persisted (state-manager, single-commit TD-VSDD-053; D-1236): architect's already-edited ADR-052 body committed — v1.15→v1.16, §Decision 5c's stale "Negative tests (cluster-5 TDD scope)" label corrected to attribute the negative-test list to devops-engineer's [D-1232-OBL-4] activation-boundary deliverable (cross-ref §Status + §Files-to-Change); NO §Decision 1–11 substance change, NO concurrency-state-machine change; v1.16 changelog row added. 3 rulings: (1) native-gate precedence CORRECTED — `bc_index_migration_admission_precheck` MUST run BEFORE `shard_cap_precheck` for BC-INDEX-path dispatches, `shard_cap_precheck` SKIPPED (not result-discarded) when migration-admission blocks (a defect the stub's `.or()`-after-both-computed composition would have let `execute_roll`'s destructive side effect outrun; BC-1.18.011 PC6(b) + ADR-052 §5a entail the fix, no spec amendment, routed to implementer T-10/T-11); (2) E-MAINTENANCE-001 → `HookResult::Block` CONFIRMED (error-taxonomy.md + hook-sdk result.rs + E-SHD Block precedent; no edit); (3) §5c classifier + its 4 dispatcher-guard amendments (§5b) + §Decision 11 executable-binding are OUT of cluster-5 T-10/T-11 TDD scope — [D-1232-OBL-4] (devops-engineer, F4 activation boundary). Cluster-5 F4 TDD IN PROGRESS: stubs committed `feature/S-25.02-b2-sharding` @ `adbc795a` (48 `todo!()` functions, crate green, baseline 3204 tests pass); test-writer authoring failing tests next. ARCH-INDEX v4.43→v4.44 (row-body amendment segment appended reflecting v1.15→v1.16; total_adrs UNCHANGED 52). Input-hash: ADR-052 `a9d309a` UNCHANGED (`--check` CLEAN both before/after — body-only edit, input-hash tracks `inputs:`, not own content). BC-INDEX/VP-INDEX/STORY-INDEX/error-taxonomy/prd.md all UNCHANGED. `pipeline:` stays in_progress. Session Resume Checkpoint refreshed: §4 item #4 ACTIVE/IN PROGRESS (stubs committed, D-1236 rulings applied, test-writer next); §5 WIP branches now lists `feature/S-25.02-b2-sharding`. [D-1232-OBL-1..4] all UNCHANGED/OPEN (ruling 3 clarifies OBL-4 scope, does not close it). OWED #2 unchanged (907-file hash sweep).** | S-25.02 F4 | 2026-09-22 |

## D-1238 — S-25.02 cluster-5 LOCAL adversary pass-1 fix-burst (NOT-CLEAN, streak → 0/3): BC-1.18.011 v1.10 (F-C5-P1-004 delivery cross-reference) + 6 code fixes on `feature/S-25.02-b2-sharding`

**Date:** 2026-09-23
**Burst type:** LOCAL adversary pass-1 fix-burst persist (state-manager, single-commit TD-VSDD-053)
**Basis:** cluster-5's own fresh F4-code LOCAL adversary cascade (0/3 baseline per D-1237 §8) ran its first pass against the T-10/T-11 code surface landed by test-writer + implementer since D-1237. Pass-1 verdict: **NOT-CLEAN** — BC-5.39.001 3-CLEAN streak resets to 0/3. 7 findings raised; all closed this burst (6 via code fix on the feature branch, 1 via a documentary BC amendment already made by product-owner prior to this burst).

**Findings (adversary pass-1, `feature/S-25.02-b2-sharding`):**

1. **F-C5-P1-001 — BLOCKER (data-loss).** `run_bc_index_migration` overwrote the canonical `BC-INDEX.md` with a 4-line stub, destroying the frontmatter block, `total_bcs`, `## Summary`, and the cross-cutting-invariants section — invisible to PC1/PC2 because both content-preservation checks exclude `## Summary` and therefore pass trivially against the zero-row stub. This is the most severe class of finding this cascade has produced: a migration that appears to satisfy its own verification gates while silently destroying the artifact it is supposed to preserve.
2. **F-002 — MEDIUM.** No end-to-end test exercised the canonical-body rebuild path at all — the actual coverage hole that let F-001 ship. Proven by a new failing test (RED, `7172bc3b`) before the fix landed.
3. **F-003 — MEDIUM.** Stale `todo!()` stub doc banners left on functions that were already fully implemented — misleading to any reader/reviewer of the diff.
4. **F-004 — MEDIUM, ADJUDICATED-DEFERRED, not a code defect.** BC-1.18.011's admission-gate precondition ("ALL mutation tool calls [Edit/Write/MultiEdit/Bash]... blocked by the native admission gate") appears unsatisfied for the Bash leg when read without visibility into D-1236 Ruling 3, which already assigned that leg to the ADR-052 §Decision 5c full-command classifier, delivered separately by devops-engineer at `[D-1232-OBL-4]`/F4 activation boundary — not by cluster-5 TDD. Closed via a documentary cross-reference in the BC itself (below), not a code change. **Correction to the finding's own location citation:** the adversary's finding text labeled this "Postcondition 6(b)"; the actual location in BC-1.18.011.md is **Precondition 6(b)** (verified this burst per POLICY 22 citation-accuracy discipline — `grep -n "^   (b)" specs/behavioral-contracts/ss-01/BC-1.18.011.md` against the `## Preconditions` section, not `## Postconditions`). The finding's substance (the delivery-split ambiguity) is unaffected by the mislabel; only the section-name citation is corrected here.
5. **F-005 — LOW.** A dangling sub-shard stub-pointer file left over from an earlier stub-architect scaffolding pass, never cleaned up once the real implementation landed.
6. **F-006 — LOW, `[process-gap]`.** Ruling-1 precedence (native-gate `bc_index_migration_admission_precheck` before `shard_cap_precheck`, per D-1236 ruling 1) was implemented but never exercised against a real binary — only unit-tested against a mock. A pure-logic helper (`resolve_shard_gate_precedence`) was extracted so the precedence rule is independently lib-testable; the test-writer follow-up (see NEXT) closes the gap by exercising it.
7. **F-007 — LOW.** `execute_canonical_path_moves` swallowed a post-rename read failure via `unwrap_or_default()` — a silent-failure pattern (CLAUDE.md Canonical Principle table: "Silent `Vec::new()` return where partial-failure data should propagate" is a SOUL.md #4 violation). Fixed to fail loud.

**Fixes (all on `feature/S-25.02-b2-sharding`):**

- RED test `7172bc3b` — new failing test proving F-002 (and, by construction, F-001: the test fails against the pre-fix stub-overwrite behavior).
- `ba07c336` — F-001 fix: canonical-body rebuild now goes through a lean-body reconstruction helper, `extract_bc_index_preamble`, instead of the destructive stub write; RED test from `7172bc3b` now GREEN.
- `902df70a` — F-007 fix: `execute_canonical_path_moves` post-rename read failure now propagates (fail-loud), replacing the `unwrap_or_default()` silent-failure pattern.
- `d2fb9abe` — F-005 fix: dangling sub-shard stub-pointer file removed.
- `e01f84cb` — F-003 fix: stale `todo!()`-era doc banners corrected on the now-implemented functions.
- `7541ddc6` — F-006 fix: `resolve_shard_gate_precedence` extracted as an independently lib-testable pure helper encoding Ruling-1 precedence.
- **F-004** closed via **BC-1.18.011 v1.9→v1.10** (product-owner, pre-existing uncommitted edit at burst start, confirmed unmodified by this burst — see BC-INDEX propagation below): Precondition 6(b) gained a "Delivery cross-reference (per D-1236 Ruling 3)" clause naming the Bash-leg/Edit-Write-MultiEdit-leg delivery split explicitly in-BC, so a fresh-context adversary reading the BC alone (without D-1236 visibility) now sees the split documented at the point of ambiguity. No Postcondition, Invariant, Edge Case, Test Vector, or VP changed — pure documentary cross-reference. This BC carries no `last_amended` frontmatter field (sibling-BC convention within `ss-01`); only `version:` and the changelog table row were bumped, confirmed by product-owner.

**Adversary pass-1 also CONFIRMED CLEAN** (no findings) on: `[D-1232-OBL-2]` APFS durability sequencing at the code level (a), Ruling-1 precedence structural unreachability of `shard_cap_precheck` post-block, the sub-shard chunk-boundary algorithm's implementation fidelity to ADR-051 §Decision 18 / VP-142, PC1/PC2/PC3a content-preservation + census + fingerprint-recheck code paths, crash-recovery/idempotency behavior, absence of test tautologies (TD-VSDD-059 paper-fix check), and absence of any new `.sh` script (POL-3/hook-bypass surface).

**Propagation this burst (state-manager):**

- **BC-INDEX.md v5.97→v5.98** — BC-1.18.011 catalog cell appended `\| v1.10 (...)`; frontmatter `version:`/`last_amended:`/`changelog:` bumped per BC-5.45.001 write-path discipline. `total_bcs` UNCHANGED 2006. This propagation also clears the transient `validate-cross-site-correspondence` workspace-test failure that the frontmatter-vs-catalog-cell version skew (BC body at v1.10, BC-INDEX cell still citing v1.9) was producing.
- **Input-hash** — BC-1.18.011.md recomputed via `plugins/vsdd-factory/bin/compute-input-hash <path> --update`: `e823637`→`84b5f96`. Follow-up `--check` CLEAN (exit 0).

**BC-5.39.001 LOCAL cluster-5 streak:** → **0/3** (pass-1 NOT-CLEAN; this is cluster-5's OWN fresh F4-code cascade resetting from its 0/3 starting baseline per D-1237 §8 — not a regression of the already-closed prose/concurrency-core cascades, which remain CLOSED at accept-at-floor / D-1233 ratification, unaffected by this burst). **NEXT:** test-writer adds the F-006 helper test (exercising `resolve_shard_gate_precedence` against a real binary invocation, not just the mock), then a fresh LOCAL adversary pass-2 (streak remains 0/3 until 3 consecutive CLEAN passes).

**Hygiene notes (recorded, not acted on destructively):**

(a) A stray git stash entry tagged `verify-pre-existing-F006-bc118011-check-*` remains on the shared stash stack (`.worktrees/S-25.02-b2-sharding`). A hook blocked its drop; inspected and confirmed a harmless byte-identical duplicate of already-committed work (the F-006 fix, `7541ddc6`). Flagged here for later cleanup — NOT dropped this burst (shared stash stack discipline: never blind-drop without re-verifying the current `stash@{n}` index by tag first).
(b) `[process-gap]` — recurring state-manager bash/sed reflex on `.factory/` mutations observed twice this session (both self-corrected before any write landed — no TD-FACTORY-HOOK-BYPASS-001 violation occurred). Root cause: the session's Bash-tool permission guards do not themselves catch a `sed -i`/redirect targeting a `.factory/` path the way the Edit/Write-tool-only discipline is stated in CLAUDE.md — the discipline currently relies on the agent's own self-check, not a mechanical gate. Anchored to the hook-hardening backlog batch already tracked in Session Resume Checkpoint §4 item 5 ("Hook-hardening batch #837–841 → rc.26") as an additional item: a PreToolUse Bash-command-pattern guard that blocks `sed`/`awk -i`/shell-redirect (`>`, `>>`) targeting any `.factory/**` path, mirroring the existing `destructive-command-guard` precedent. No new story ID allocated this burst — anchor is the existing rc.26 hook-hardening batch, not a bare "later."

**Sidecar note:** this burst's commit also folds in pre-existing uncommitted `sidecar-learning.md` + `regression-state.json` deltas to leave the `.factory/` worktree clean at commit time — incidental inclusions, not this burst's own content.

**STATE.md this same burst:** `pipeline:` stays `in_progress` (unchanged). Phase Progress row appended: `S2502-CLUSTER5-LOCAL-ADV-PASS1-FIX-BURST 2026-09-23 (D-1238)`. Session Resume Checkpoint refreshed: §3/§4/§8 sub-state advanced — cluster-5 F4 TDD code green including sub-shard chunking; LOCAL adversary cascade IN PROGRESS (pass-1 fixed, streak 0/3, pass-2 next); feature branch @ `7541ddc6`. STATE.md v10.69→v10.70.

**OWED (unchanged):** OWED #2 (907-file `compute-input-hash --scan --update` sweep). `[D-1232-OBL-1..4]` all remain OPEN/PENDING, cluster-5-scoped, UNCHANGED by this burst.

### Canonical 6-column row (STATE.md Decisions Log)

| D-1238 | D-1238-S2502-CLUSTER5-LOCAL-ADV-PASS1-FIX-BURST | **S-25.02 cluster-5 LOCAL adversary pass-1 fix-burst persisted (state-manager, single-commit TD-VSDD-053; D-1238): pass-1 = NOT-CLEAN (BC-5.39.001 streak → 0/3). 7 findings: F-C5-P1-001 BLOCKER (data-loss — `run_bc_index_migration` overwrote canonical BC-INDEX.md with a 4-line stub, destroying frontmatter/`total_bcs`/`## Summary`/cross-cutting-invariants, invisible to PC1/PC2 which exclude `## Summary` and pass trivially on the zero-row stub); F-002 MEDIUM (no end-to-end canonical-body test — the coverage hole that let F-001 ship); F-003 MEDIUM (stale `todo!()` stub doc banners on implemented functions); F-004 MEDIUM (BC-1.18.011 Precondition 6(b) Bash-leg delivery split — ADJUDICATED-DEFERRED to `[D-1232-OBL-4]` per D-1236 Ruling 3, closed via documentary cross-ref, NOT a code defect; finding mislabeled "Postcondition 6(b)" by the adversary — actual location Precondition 6(b), corrected this burst per POLICY 22 citation-accuracy verification); F-005 LOW (dangling sub-shard stub-pointer file); F-006 LOW `[process-gap]` (Ruling-1 precedence untested against a real binary — mock-only); F-007 LOW (`execute_canonical_path_moves` swallowed a post-rename read failure via `unwrap_or_default()`, a silent-failure pattern). ALL fixed on `feature/S-25.02-b2-sharding`: RED test `7172bc3b` (F-002, proves F-001), fixes `ba07c336` (F-001 lean-body rebuild via `extract_bc_index_preamble`), `902df70a` (F-007 fail-loud), `d2fb9abe` (F-005 stub-pointer removal), `e01f84cb` (F-003 doc-banner correction), `7541ddc6` (F-006 `resolve_shard_gate_precedence` lib-testable helper). F-004 closed via BC-1.18.011 v1.9→v1.10 (product-owner, pre-existing edit, confirmed unmodified). Adversary CONFIRMED CLEAN on: OBL-2(a) durability, Ruling-1 unreachability, sub-shard algorithm fidelity to ADR-051 §Decision 18/VP-142, PC1/PC2/PC3a, crash/idempotency, no test tautologies (TD-VSDD-059), no new `.sh`. BC-INDEX v5.97→v5.98 (BC-1.18.011 cell v1.9→v1.10; `total_bcs` UNCHANGED 2006; this propagation also clears the transient `validate-cross-site-correspondence` version-skew failure). Input-hash: BC-1.18.011.md `e823637`→`84b5f96` (`compute-input-hash --update`, `--check` CLEAN). BC-5.39.001 LOCAL cluster-5 streak → 0/3 (fresh F4-code cascade, distinct from the already-CLOSED prose/concurrency-core cascades). Hygiene notes recorded (not acted on): (a) stray shared-stash entry `verify-pre-existing-F006-bc118011-check-*` — harmless byte-identical duplicate, flagged for cleanup; (b) `[process-gap]` recurring state-manager bash/sed reflex on `.factory/` mutations (2 instances, both self-corrected, no violation) — anchored to the existing rc.26 hook-hardening backlog batch as an additional PreToolUse `sed`/redirect-on-`.factory/**` guard item. `pipeline:` stays in_progress. NEXT: test-writer adds F-006 helper test exercising `resolve_shard_gate_precedence` against a real binary, then fresh LOCAL adversary pass-2 (streak 0/3). OWED #2 unchanged (907-file hash sweep). `[D-1232-OBL-1..4]` all UNCHANGED/OPEN.** | S-25.02 F4 | 2026-09-23 |

## D-1239 — S-25.02 cluster-5 OBL-1 crash-recovery systematic discharge (ADR-052 v1.17 §Decision 12; Kani 7/7 PROVED + fault-injection 30/30 green; discharges [D-1232-OBL-1])

**Date:** 2026-09-23
**Burst type:** OBL-1 discharge arc checkpoint persist (state-manager, single-commit TD-VSDD-053)
**Basis:** cluster-5 LOCAL adversary pass-2 (continuing directly from the D-1238 pass-1 fix-burst) found 2 HIGH + 1 MED crash-recovery defects in the code surface — fail-open on a coexisting txn, unimplemented forward recovery / dead `decide_intel_log_recovery`, incomplete-staging never discarded — fixed at `93ce4fde`/`2257277a`. Rather than continue arm-by-arm pass-fix-pass patching, the human approved a full systematic OBL-1 discharge: research-agent investigation feeding an architect design, with governance verdict REFINES/IMPLEMENTS (no re-ratification required — ADR-052 §7b already mandated WAL-before-rename).

**Refactor (`cdd9b5f2`..`2ac74914`):** `Fs` trait seam (`migration_fs.rs`); a total WAL-ordered `recover()` as the single resume dispatcher, subsuming the pass-2 arm-patches and closing the fail-open structurally via full-slice `classify_txn_records`; intent-durable-before-rename WAL fix; O-5 OPEN/DRAINING/reservation drain wiring (`tool_use_id` confirmed present in `HookPayload.extra`).

**Fault-injection suite:** 30/30 green (fail 0.5.1 + child-process abort). Found and fixed **5 MORE** crash-consistency defects during hardening, including a WAL-reorder-introduced silent-false-success `count:0`, a `completed.json` gate-lockout, and a dead `pointer_swap` commit seam (`3e5b4a8e`/`1a92b968`).

**Kani:** 7/7 PROVED (`recover()` totality, fail-closed safety, transition inductive invariants, INV-GATE-TXN admission quiescence, pointer-swap crash-atomicity+WAL, idempotence; `8df16e4d`). Also fixed the repo's own pre-existing VP-077 Kani harnesses (broken since Kani 0.68's `kani::assert` fn change, never caught because no Kani CI existed) and added `.github/workflows/kani.yml` (`31eefb58`, hardened with a positive-coverage `EXPECTED_PROOFS=7` assertion per adversary finding F-C5-P3-001).

**Pass-3 LOCAL adversary:** NOT-CLEAN, but cleared ALL crash-recovery correctness under fresh analysis — only 1 MED (kani.yml coverage) + 1 LOW (stale test comments), both fixed at `fb39c267`.

**[D-1232-OBL-1] DISCHARGED** (fault-injection + Kani, CI-gated). ADR-052 §Decision 12 (v1.16→v1.17) documents this verification architecture — see `specs/architecture/decisions/ADR-052-native-migration-cli-bash-tool-allowlist-sanctioned-execution-path.md`, propagated same burst to ARCH-INDEX v4.45→v4.46 (SS-01 row + frontmatter + changelog prepend). ADR-052 input-hash recomputed `a9d309a`→`02d5adb` (`compute-input-hash --update`; `--check` CLEAN).

**BC-5.39.001 LOCAL cluster-5 streak:** pass-3 at asymptotic floor (no crash-recovery correctness defects across 3 fresh passes); pass-4 confirming pending.

**Drift Items recorded this burst (human-visible, tracked, not lost):**
(a) **[D-1239-DRIFT-001]** `reconcile_stale_admission_gate` wired-deferred — the gate self-heals on migration-retry (fault-injection-confirmed), not on the next PreToolUse as originally assumed; a robustness item, anchored to the cluster-5 follow-up or the hook-hardening backlog.
(b) **[D-1239-DRIFT-002]** armed-activation-manifest reader unimplemented — `recover()`'s ManifestStatus arms route conservatively fail-closed pending this reader; anchored to the activation-boundary scope alongside [D-1232-OBL-2]/[D-1232-OBL-3]/[D-1232-OBL-4].

**Sidecar note:** this burst's commit also folds in pre-existing uncommitted `sidecar-learning.md` + `regression-state.json` telemetry deltas to leave the `.factory/` worktree clean at commit time — incidental inclusions, not this burst's own content.

**STATE.md this same burst:** `pipeline:` stays `in_progress` (unchanged). Phase Progress row appended: `S2502-CLUSTER5-OBL1-DISCHARGE 2026-09-23 (D-1239)`. Blocking Issues: `[D-1232-OBL-1]` marked **DISCHARGED**. Session Resume Checkpoint refreshed: §4 cluster-5 sub-state — F4 code complete including sub-shard; OBL-1 discharged via fault-injection 30/30 + Kani 7/7; feature branch `fb39c267`; NEXT = adversary pass-4 confirming, then demo, then PR. STATE.md v10.71→v10.72.

**OWED (unchanged):** OWED #2 (907-file `compute-input-hash --scan --update` sweep). `[D-1232-OBL-2]`/`[D-1232-OBL-3]`/`[D-1232-OBL-4]` remain OPEN/PENDING, activation-boundary-scoped, UNCHANGED by this burst.

### Canonical 6-column row (STATE.md Decisions Log)

| D-1239 | D-1239-S2502-CLUSTER5-OBL1-DISCHARGE | **S-25.02 cluster-5 OBL-1 crash-recovery systematic discharge. Pass-2 LOCAL adversary found 2 HIGH + 1 MED crash-recovery defects (fail-open on coexisting txn; unimplemented forward recovery/dead `decide_intel_log_recovery`; incomplete-staging never discarded) — fixed `93ce4fde`/`2257277a`. Human approved a full systematic OBL-1 discharge (research-agent investigation → architect design; governance verdict REFINES/IMPLEMENTS, no re-ratification — ADR-052 §7b already mandated WAL-before-rename). Refactor (`cdd9b5f2`..`2ac74914`): `Fs` trait seam (`migration_fs.rs`), total WAL-ordered `recover()` as the single resume dispatcher (subsumes the pass-2 arm-patches, closes the fail-open structurally via `classify_txn_records` full-slice), intent-durable-before-rename WAL fix, O-5 OPEN/DRAINING/reservation drain wiring (`tool_use_id` confirmed in `HookPayload.extra`). Fault-injection suite 30/30 (fail 0.5.1 + child-process abort; found+fixed 5 MORE crash-consistency defects incl. a WAL-reorder-introduced silent-false-success `count:0`, a `completed.json` gate-lockout, and a dead `pointer_swap` commit seam — `3e5b4a8e`/`1a92b968`). Kani 7/7 PROVED (recover totality, fail-closed safety, transition inductive invariants, INV-GATE-TXN admission quiescence, pointer-swap crash-atomicity+WAL, idempotence; `8df16e4d`) — ALSO fixed the repo's pre-existing VP-077 Kani harnesses (broken since Kani 0.68's `kani::assert` fn change, never caught because NO Kani CI existed) and added `.github/workflows/kani.yml` (`31eefb58` hardened with positive-coverage `EXPECTED_PROOFS=7` assertion, adversary F-C5-P3-001). Pass-3 LOCAL adversary NOT-CLEAN but cleared ALL crash-recovery correctness under fresh analysis — only 1 MED (kani.yml coverage) + 1 LOW (stale test comments, `fb39c267`), both fixed. ADR-052 §Decision 12 documents the verification architecture. **[D-1232-OBL-1] DISCHARGED** (fault-injection + Kani, CI-gated). BC-5.39.001 LOCAL cluster-5 streak: pass-3 at asymptotic floor (no crash-recovery correctness defects across 3 fresh passes); pass-4 confirming pending.** | S-25.02 F4 | 2026-09-23 |

## D-1240 — S-25.02 cluster-5 convergence + merge declaration (PR #842 squash-merged to develop as `ddd99212`; POL-14 BC-1.18.010/011 draft→active; merged_count 122→123)

**Date:** 2026-09-25
**Burst type:** post-merge state burst (state-manager, single-commit TD-VSDD-053)

Continuing directly from the D-1239/SESSION-WRAP-PAUSE-2026-09-24 checkpoint (STATE.md v10.73, pipeline PAUSED, PR #842 open at CI 16/17 green with the `build-dispatcher (windows-x64)` blocker outstanding). Between the wrap and this burst, the Windows CI fix was pursued to completion, a Finding C spec-conflict was adjudicated, and PR #842 was squash-merged into `develop` as `ddd99212` (base `ebd16f79`). This entry formally declares the cluster-5 F4-code LOCAL adversary convergence (pass-4 CLEAN, accept-at-floor per D-386 Option C, human-approved 2026-09-24 — the decision-log codification of that declaration was itself deferred to this post-merge burst per the D-1239/wrap checkpoint) AND records the merge. Three sub-decisions from the intervening Windows-fix work are codified below as D-1240(a)/(b)/(c).

- **(i) Convergence declaration (ratifying the 2026-09-24 accept-at-floor):** cluster-5's own fresh F4-code LOCAL adversary cascade — pass-1 (BLOCKER data-loss, D-1238) → pass-2/pass-3 (OBL-1 crash-recovery systematic discharge, D-1239) → **pass-4 CLEAN** — is declared CONVERGED at accept-at-floor per D-386 Option C. This is a DIFFERENT axis from the ADR-052 concurrency-core prose cascade (trajectory-tail →1→1→2→1 LENGTH=4, unaffected, tracked separately since D-1230).
- **(ii) Merge declaration:** PR #842 squash-merged into `develop` as `ddd99212` (base `ebd16f79`); feature branch `feature/S-25.02-b2-sharding` deleted post-merge. Two code fixes shipped: the `StdFs::fsync_file` read-only-handle/`FlushFileBuffers` fix (D-1240(a)) and a second Windows-path fix in `rotate_changelog_at`. One attempted fix (Item 3, `indeterminate_marker::block_if_marker_check` tightened fail-open→fail-closed) was REVERTED pre-merge per D-1240(c). CI green on all applicable platforms at merge. POL-14 auto-promotion this burst: BC-1.18.010 + BC-1.18.011 `status`/`lifecycle_status` draft→active (both stay v1.10, no body content change) — BC-INDEX v5.98→v5.99. `merged_count` 122→123 (genuine BC-cluster feature delivery, cluster-1/2/3/4 precedent D-1173/D-1186/D-1206/D-1212 applied).

**D-1240(a) — Windows root-cause CORRECTION (supersedes the session-wrap-pause v10.73 diagnosis):** The session-wrap-pause checkpoint (D-1239/v10.73) diagnosed the `build-dispatcher (windows-x64)` CI failure as `write_atomic_strict_durable` (`crates/last-amended-migrate/src/atomic_write.rs`) holding a temp-file handle open across `std::fs::rename`. During the fix session that diagnosis was itself superseded: an intermediate theory (an antivirus scan-handle race — the SAME misdiagnosis class already codified as a lesson pattern in this cycle, see L-BB-* entries on premature-theorizing) was entertained and speculatively hardened against BEFORE a fresh-eyes pass pulled the actual CI failure log and read the exact code path. The TRUE root cause is deterministic and unrelated to any antivirus interaction: `StdFs::fsync_file` opens the target file **read-only**, then calls `FlushFileBuffers` on that handle — Windows' `FlushFileBuffers` requires a handle opened with write access (`GENERIC_WRITE`); a read-only handle fails deterministically on every invocation, on every Windows CI run, with no timing dependency whatsoever. This is a plain API-contract defect, not a race condition. **Discipline codified:** pull the ACTUAL CI failure log and read the exact failing code path BEFORE theorizing a root cause — a plausible-sounding external-interference theory (AV, disk contention, timing) must never be accepted as the working diagnosis while the log itself is unread. See L-BB-D1240-diagnosis-discipline-pull-log-before-theorizing (lessons.md).

**D-1240(b) — Human scope decision: REVERT the speculative 8-primitive AV-retry sweep:** While the AV-race theory was still the working hypothesis, an implementer burst produced a speculative, comprehensive retry/backoff sweep across 8 durability primitives (open/rename/fsync/flush call sites) intended to tolerate transient AV-held-handle contention, landing as local commit `465585e3` (unpushed). Once D-1240(a)'s deterministic root cause was found, the human directed **REVERTING** commit `465585e3` in favor of a minimal, targeted fix (correct `fsync_file`'s handle-open mode) — rejecting the broader speculative hardening because it treated a symptom class (transient contention) that does not exist for this defect, added retry-loop complexity with no discharging test rationale, and would have shipped unverified speculative code under time pressure. Recovery used the hook-safe local-revert recipe (destructive-command-guard blocks a raw `git reset --hard`): `git stash` → `git reset --soft <base>` → `git stash` again to restore only the wanted (non-`465585e3`) changes, yielding a fast-forward-safe clean revert without invoking any blocked destructive command. See L-BB-D1240-destructive-command-guard-local-revert-recipe (lessons.md).

**D-1240(c) — Finding C resolution: BC-1.18.002 fail-open REAFFIRMED (spec wins); Item 3 reverted; S-25.07 filed:** During the same fix cycle, a PR-diff-level review (Finding C) flagged `indeterminate_marker::block_if_marker_check`'s fail-open behavior on a non-`NotFound` marker-read I/O error as a CWE-703 (Improper Check for Exceptional Conditions) weakness and an implementer change (Item 3) tightened it to fail-CLOSED. This is adjudicated INCORRECT: BC-1.18.002 v1.8 Invariant 2 / EC-030 **deliberately** requires fail-open on exactly this error class — the documented rationale is that failing closed here risks an unclearable self-lock (the same permission fault that makes the marker unreadable to the gate could also make it un-`rm`-able via the operator's break-glass escape hatch, violating INV6's Ungated-Escape guarantee). The reviewer, walled off from `.factory/` spec content per the PR-review information-asymmetry design, could not see this deliberate invariant and mis-flagged a considered tradeoff as a defect. Per the CLAUDE.md Standing Rule ("for code-vs-spec conflicts, the spec wins") and Canonical Principle Rule 3, Item 3 was **REVERTED** pre-merge, and — because this IS a genuine, legitimate cross-cutting security-vs-availability tradeoff warranting joint product-owner + architect + security-reviewer re-evaluation, not a silent one-PR fix — **S-25.07** ("Revisit BC-1.18.002 INV2/EC-030 Marker-Read I/O-Error Fail-Open Posture") was filed as the human-directed follow-up (E-25; 8 pts; P2; draft; `behavioral_contracts: [BC-1.18.002]`; `verification_properties: [VP-105]`; input-hash `5603710`, VERIFIED). All three Canonical Principle Rule 3 conditions are satisfied: (1) explicit human direction ("revert + follow-up story"), (2) concrete future dependency (joint specialist review, not answerable by a single agent in the original PR's scope), (3) attachment to the specific story S-25.07 (not a bare "later"). See L-BB-D1240-spec-before-fix-verify-deliberate-invariant (lessons.md).

**Propagation this burst (state-manager):**

- **BC-1.18.010.md / BC-1.18.011.md** — `status`/`lifecycle_status` draft→active (both files); `version:` UNCHANGED v1.10 (pure status flip, no body content change). Input-hashes recomputed (`compute-input-hash --update`, pre-existing OWED-#2-class drift on both files also cleared incidentally): BC-1.18.010.md `6b1de10`→`1684ab3`; BC-1.18.011.md `84b5f96`→`092a7b3`. `--check` CLEAN on both.
- **BC-INDEX.md v5.98→v5.99** — BC-1.18.010 + BC-1.18.011 catalog-row STATUS cells draft→active; `total_bcs` UNCHANGED 2006. Input-hash recomputed (pre-existing drift also cleared incidentally): `f3ea943`→`c22de28`, `--check` CLEAN.
- **STORY-INDEX.md v4.476→v4.477** — new catalog row S-25.07; E-25 blockquote input-hashes list gained `S-25.07=5603710` and re-synced `E-25=b8e3f20`→`4a29767`. Input-hash recomputed: `7cc0c23`→`350ca7f`, `--check` CLEAN.
- **E-25 epic file v1.1→v1.2** — S-25.07 registered (Stories table row + Changelog table row); S-25.06 BACKFILLED into the Stories table (registered in STORY-INDEX at D-1209/2026-09-12 but never propagated to this epic file — a TD-VSDD-060 sibling-gap discovered this burst). `story_count` corrected 5→7 in one motion (the intervening 5→6 step for S-25.06 was never actually applied on disk). Total points ~85→~101. Input-hash recomputed: `feb39e0`→`4a29767`, `--check` CLEAN.
- **`code-delivery/S-25.02/pr-review-cluster-5-consolidated.md`** — NEW consolidated review-trail file persisting the full Phase 1 (5 pr-reviewer cycles + 3 security passes, converging at head `4cbcbf3d`) and Phase 2 (Windows re-failure → misdiagnosis → correction → revert → minimal fix → Finding C → merge, converging at head `6863611f`) review history, closing the cross-cycle review-trail traceability gap (L-BB-D1240-review-trail-persistence, lessons.md).
- **Sidecar note:** this burst's commit also folds in pre-existing uncommitted `sidecar-learning.md` deltas (session-end telemetry entries) present in the `.factory/` worktree at burst start — incidental inclusions, not this burst's own content, consistent with the D-1238/D-1239 precedent of folding in ambient telemetry churn.

**Lessons codified this burst (lessons.md), all `[process-gap]`:** L-BB-D1240-diagnosis-discipline-pull-log-before-theorizing; L-BB-D1240-sibling-sweep-all-fresh-file-operations; L-BB-D1240-spec-before-fix-verify-deliberate-invariant; L-BB-D1240-tempfile-hygiene-use-scratchpad-not-shared-tmp; L-BB-D1240-destructive-command-guard-local-revert-recipe; L-BB-D1240-review-trail-persistence.

**STATE.md this same burst:** `pipeline:` PAUSED→**in_progress** (session resumed to active delivery; cluster-5 delivered, cluster-6 next per D-1170's sequencing). Phase Progress row appended: `S2502-CLUSTER5-DELIVERY-MERGE-BURST 2026-09-25 (D-1240)`. Blocking Issues: `[PR-842-CI] build-dispatcher (windows-x64)` row marked **RESOLVED** (fixed pre-merge). `merged_count` 122→123; develop HEAD `ebd16f79`→`ddd99212`. Active Branches: `feature/S-25.02-b2-sharding` marked MERGED+DELETED. Session Resume Checkpoint refreshed (prior v10.73 checkpoint archived verbatim to `cycles/v1.0-brownfield-backfill/session-checkpoints.md`). STATE.md v10.73→v10.74.

**OWED (unchanged):** OWED #2 (907-file `compute-input-hash --scan --update` sweep). `[D-1232-OBL-2]`/`[D-1232-OBL-3]`/`[D-1232-OBL-4]` remain OPEN/PENDING, activation-boundary-scoped, UNCHANGED by this burst (migration shipped DORMANT; these gate F4 *activation*, not the merge). **NEW:** S-25.07 OPEN (E-25; joint review of BC-1.18.002 INV2/EC-030 fail-open posture). Cluster-6 (migrations) is next per D-1170's sequencing.

### Canonical 6-column row (STATE.md Decisions Log)

| D-1240 | D-1240-S2502-CLUSTER5-CONVERGENCE-MERGE-DECLARATION | **S-25.02 cluster-5 convergence + merge declaration: cluster-5's own fresh F4-code LOCAL adversary cascade (pass-1 BLOCKER data-loss D-1238 → pass-2/pass-3 OBL-1 discharge D-1239 → pass-4 CLEAN) declared CONVERGED at accept-at-floor per D-386 Option C, ratifying the 2026-09-24 human approval. PR #842 squash-merged into develop as `ddd99212` (base `ebd16f79`); feature branch `feature/S-25.02-b2-sharding` deleted. **D-1240(a) Windows root-cause CORRECTION:** superseded the session-wrap-pause `write_atomic_strict_durable` rename-with-open-handle diagnosis — an intervening antivirus-scan-race theory (misdiagnosis) was entertained and speculatively hardened against before a fresh-eyes log pull found the TRUE deterministic defect: `StdFs::fsync_file` opened the file read-only then called `FlushFileBuffers`, which Windows requires a write-access handle for; zero timing dependency. **D-1240(b) human scope decision:** REVERTED the speculative 8-primitive AV-retry sweep (`465585e3`, unpushed) in favor of the minimal `fsync_file` fix, using the hook-safe `git stash`→`git reset --soft <base>`→`git stash` local-revert recipe (destructive-command-guard blocks raw `--hard`). **D-1240(c) Finding C resolution:** a PR-diff reviewer (walled off from `.factory/` specs) flagged `indeterminate_marker::block_if_marker_check`'s fail-open-on-I/O-error as CWE-703 and an implementer change (Item 3) tightened it fail-closed; ADJUDICATED INCORRECT — BC-1.18.002 v1.8 Invariant 2/EC-030 deliberately requires fail-open (unclearable-self-lock/INV6 avoidance); per the Standing Rule (spec wins) Item 3 was REVERTED pre-merge and **S-25.07** filed as the human-directed joint-review follow-up (Canonical Principle Rule 3, all 3 conditions met). Two net code fixes shipped: `fsync_file` (D-1240(a)) + a second Windows-path fix in `rotate_changelog_at`; CI fully green at merge. POL-14: BC-1.18.010 + BC-1.18.011 status/lifecycle_status draft→active (v1.10 UNCHANGED both) — BC-INDEX v5.98→v5.99. STORY-INDEX v4.476→v4.477 (S-25.07 registered). E-25 epic file v1.1→v1.2 (S-25.07 registered + S-25.06 backfilled into Stories table, a TD-VSDD-060 sibling-gap from S-25.06's 2026-09-12 registration never having propagated to the epic file; story_count corrected 5→7 in one motion). `merged_count` 122→123. 6 `[process-gap]` lessons codified (L-BB-D1240-diagnosis-discipline / sibling-sweep-all-fresh-file-operations / spec-before-fix / tempfile-hygiene / destructive-command-guard-recipe / review-trail-persistence). Consolidated review trail persisted to `code-delivery/S-25.02/pr-review-cluster-5-consolidated.md`, closing a cross-cycle traceability gap. `pipeline:` PAUSED→in_progress. Cluster-6 (migrations) next per D-1170's sequencing.** | S-25.02 F4→Merge | 2026-09-25 |
